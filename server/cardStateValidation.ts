...
export function validateCardStateExtensions(state:FinanceData['state']){
  ...
  for(const card of state.cards??[]){
    ...
    if(card.kind==='credit'&&card.active!==false)activeCreditCount+=1;
    if(card.active===false&&!card.archivedAt){
      // Legacy archived cards are still valid; the timestamp is added on the next mutation.
    }
  }
  // The current ledger has one synthetic credit-card liability. Two active
  // card identities would make purchases/repayments ambiguous, so fail closed.
  if(activeCreditCount>1)invalid();
