...
- [ ] Migrate bounded safe adopter groups incrementally. Batch 1 (`ConfirmDialog`, `MoneyEditDialog`, reusable `CardCreateDialog` actions) merged via PR #363. Batch 2 (`DesktopUpdatePanel`, `PageErrorBoundary`, `PersistenceNotice`, `AccountMetadataSettings`) merged via PR #364. Batch 3 (`AppDateInput`, `AppSelectInput`, `CommandPalette` generic icon actions) merged via PR #365. Batch 4 (Lending generic primary/secondary/ghost/icon actions) merged via PR #366. Batch 5 is active in PR #367 and migrates only Recurring generic page-heading, mobile payment and editor primary/secondary/icon actions while preserving row, menu and history controls as intentional raw domain/composite controls.
...
Active delivery: issue #357 / PR #367 / branch `chore/357-button-adopters-batch-5`.
...
- **Active PR:** #367 — `Code health: migrate Recurring shared Button adopters batch 5`.
- **Branch/base:** `chore/357-button-adopters-batch-5`, created exactly from verified `develop@68ebce7c337f8996adb2d809edf15d59a8e1e699`.
- **Current implementation commits:** `451faabf424557f03a279430928dd4a7de168685` migrates the five Recurring generic actions to four shared `Button` instances plus one shared `IconButton`; `5b48d3bb55912324e79d74ebf68bfacb3e3715cd` adds a source-level ownership guard. Existing explicit button types, accessible names, handlers, modal-focus contract and compatibility class geometry remain unchanged.
- **Intentional Recurring raw controls:** desktop row payment/edit/pause/stop controls, mobile action-menu summary/buttons and inactive-history row controls remain native/domain-owned because converting them to `IconButton` or generic shared actions would introduce a different visual contract.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** PR #367 exact-head application/API/CodeQL/cross-engine/performance/Windows/rendered validation is required before merge. Inspect fresh `recurring-cadence` desktop, editor desktop and editor mobile evidence, including mobile touch-target geometry and modal focus/close behavior.
- **Intentional exceptions remain:** card-design radio options, Lending domain/composite controls, Recurring row/menu/history controls and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** complete PR #367 exact-head required gates, inspect fresh Recurring desktop/editor desktop/editor mobile evidence, remove any generated `visual-qa/**` source churn while preserving artifacts as evidence, re-run final exact-head gates if the branch moves, resolve any real review/regression, then squash-merge to `develop` only if green and verify the integration push.
