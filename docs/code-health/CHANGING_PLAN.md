...
Active delivery: issue #357 / branch `chore/357-button-adopters-batch-15`.
...
- **Verified Batch-13 integration:** PR #375 squash-merged to `develop@fa6ad460ab23dd826a5fc93339b766a54d92b172` (tree `76685cd0598f07d95bfbd7fbefbc491486c9b4e7`). Batch 13 migrated **16/16** Credit Card canonical-hook controls: **6/6 primary**, **8/8 secondary**, **2/2 icon-only**. Final exact-head required gates were **5/5 green**: CI `34624469314`, CodeQL `34624469328`, Cross-engine `34624469310`, Performance `34624469306`, Windows Desktop `34624469316`. Fresh Credit desktop/mobile and desktop statement-payment dialog evidence was personally inspected; representative selected views were **3/3** and review blockers were zero. Post-merge integration is **3/3 green**: CI `34625830197`, CodeQL `34625830156`, Windows Desktop `34625830164`.
- **Verified Batch-14 integration:** PR #376 squash-merged to `develop@fd506d96727e65a5875358515ff2b4b321ddf630` (tree `aebfce259df6d00ca436a8682be50f34a2859c2f`). Batch 14 migrated **7/7** Savings canonical target/dialog controls: **2/2 primary**, **4/4 secondary**, **1/1 icon-only**. Final exact-head required gates were **5/5 green** with zero review blockers. Post-merge integration is **3/3 green** after one targeted retry of a transient rendered-QA timeout: CI `34641472990` attempt 2, CodeQL `34641473010`, Windows Desktop `34641473087`; the successful CI attempt also passed rendered frontend QA, audits and evidence upload without a source change.
...
- **Active Batch-15 branch/base:** `chore/357-button-adopters-batch-15`, based exactly on verified `develop@fd506d96727e65a5875358515ff2b4b321ddf630`.
- **Batch-15 target controls:** **24/24 prepared** across five production surfaces: Dashboard **1/1**, Attention **4/4**, BudgetRuleSettings **6/6**, TransactionRulesWorkspace **10/10**, LegacyConfirmationPanel **3/3**. Only controls already owning canonical `save-button`, `secondary`, or `icon-button` semantics are migrated.
- **Preserved raw exceptions in Batch 15:** Dashboard account/domain navigation actions, Attention group/domain snooze-dismiss actions, Legacy review-decision actions and split-part destructive action, Savings mode cards, and other documented domain composites remain raw; finance routing and source semantics are unchanged.
- **Current source scope:** **8/8 intended files** including five production surfaces, the existing Attention target guard, a dedicated Batch-15 ownership guard and this plan checkpoint. No CSS, finance/accounting semantics, auth, API, database, persistence, routes, Windows, release or deploy changes.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** Batch 15 still requires atomic branch publication, narrow/source validation, draft PR, exact-head application/API/CodeQL/cross-engine/performance/Windows gates, fresh rendered evidence for affected routed surfaces where emitted, review-thread verification and generated snapshot cleanup if needed.
- **Intentional exceptions remain:** card-design radio options, Cards card-domain/composite controls, Lending domain/composite controls, Recurring row/menu/history controls, Loans row actions, Planning row/horizon/kind controls, QuickAdd kind/frequent/split controls, taxonomy selectors/row controls, receipt destructive/domain controls, Credit Card deck/navigation/row/destructive/picker-specific controls, Savings mode cards, Dashboard domain/account controls, Attention domain-specific actions, Legacy review-decision/destructive controls and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** publish the exact Batch-15 atomic tree once its **8/8** file diff is verified, open the bounded draft PR, run one exact-head validation cycle, inspect fresh representative evidence without overclaiming unavailable states, resolve real regressions/review threads, then squash-merge to `develop` only when final-head gates are green and verify the integration push.
