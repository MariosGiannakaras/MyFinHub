...
- [ ] Batch 3: migrate only generic application-chrome elevation hosts in `AppShell` plus the matching loading-shell sidebar to `Surface`, preserving exact semantic tags/classes and leaving overlay/state-specific `neo-*` ownership specialized.
...
Active Batch 3: branch `chore/357-surface-batch-3`, based exactly on verified `develop@4a9743949f4499f4e45ac90b91a095030b54bb42`. Initial source/test milestone `af5aec259a99257dc63609ef1ff568d524e0ccf9` (tree `03a6d0272e358b414637012166f5b7e8104f3dcd`) moves only the desktop sidebar, sticky topbar, mobile navigation and matching loading-skeleton sidebar to `Surface` while preserving the emitted `aside` / `header` / `nav` tags and exact compatibility class order. The history palette, mobile-more dialog, history rows and dynamic save-state inset remain specialized raw `neo-*` owners. No CSS, finance/accounting, auth, API, persistence, routing, database, Windows packaging, release/deploy or production-data behavior is changed.
...
- **Verified integration base:** `develop@4a9743949f4499f4e45ac90b91a095030b54bb42` (tree `2781e41c6d10f3db582fa6cb5cf39fc20fa27992`); Batch-2 post-merge CI `34754599030`, CodeQL `34754599047`, and Windows Desktop `34754599020` are green.
- **Active delivery:** `chore/357-surface-batch-3`, based exactly on that verified integration commit.
- **Batch-3 source scope:** move only the `AppShell` desktop sidebar (`raised`), sticky topbar (`flat`), mobile navigation (`raised`) and the matching `AppSkeleton` sidebar (`raised`) to canonical `Surface`. Preserve exact emitted semantic tags, existing class hooks, nav/ARIA behavior, quick-entry/command/history actions and loading-shell geometry. Keep the command/history/mobile-more overlays plus dynamic save-state/history-row inset semantics raw and specialized.
- **Initial Batch-3 milestone:** `af5aec259a99257dc63609ef1ff568d524e0ccf9` (tree `03a6d0272e358b414637012166f5b7e8104f3dcd`); base→milestone is 3 intended files, +24/−3, 1 commit ahead / 0 behind before durable plan sync.
- **Eligibility boundary:** `PageErrorBoundary` remains out because its focus-restoration ref contract would require deliberate `Surface` ref forwarding. Login/MFA/security surfaces, command/history overlays, finance/account/payment/attention/loan/lending/recurring/planning/report cards and control shells remain specialized/domain-owned rather than migration targets by class name alone.
- **Next action:** open a draft Batch-3 PR to `develop`, verify exact base→head scope, run required CI/CodeQL/Cross-engine/Performance/Windows gates and inspect fresh shell desktop/mobile/loading evidence. Resolve source-test, generated visual persistence or review blockers at root cause; then mark ready, wait for ready-triggered required checks, squash-merge only to `develop`, and require fresh post-merge CI + CodeQL + Windows 3/3 before the next Stage-4 write batch.
