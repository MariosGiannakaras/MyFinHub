...
Active delivery: issue #357 / branch `chore/357-button-adopters-batch-11`.
...
- **Active Batch-11 branch/base:** `chore/357-button-adopters-batch-11`, created exactly from verified `develop@d2614c266025fb29c44afbe7265add58401c510e`.
- **Current Batch-11 implementation commits:** `1a01b05259d1cb7761b9027670a9975fce34f85a` migrates `QuickAdd` generic dialog close/cancel/submit to one shared `IconButton` plus two shared `Button` instances while preserving explicit button types, accessible close name, handlers, dirty/discard flow, modal focus and finance-event semantics. `3e2888fc46620697eacbe3fb2ca7e0d31b4e47f3` adds a dedicated QuickAdd source ownership guard.
- **Batch-11 target controls:** **3/3 migrated** in the current implementation: dialog close, footer cancel, footer submit. QuickAdd kind-grid choices, frequent presets, split-part remove and split-add controls remain intentionally raw domain/composite controls.
- **Current source scope:** 2/2 intended files before this plan checkpoint (`QuickAdd.tsx`, dedicated source guard); no CSS, finance/accounting, auth, API, database, persistence, routes, Windows, release or deploy changes.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** Batch 11 still requires a draft PR, exact-head application/API/CodeQL/cross-engine/performance/Windows gates, fresh QuickAdd desktop/mobile/dialog rendered evidence where emitted, generated snapshot cleanup if needed, and review-thread verification before merge.
- **Intentional exceptions remain:** card-design radio options, Cards card-domain/composite controls, Lending domain/composite controls, Recurring row/menu/history controls, Loans row actions, Planning row/horizon/kind controls, BudgetRuleSettings budget/rule row controls, QuickAdd kind/frequent/split controls and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** open the bounded Batch-11 draft PR, run exact-head validation, inspect fresh QuickAdd evidence without overclaiming unavailable states, clean any generated snapshot churn, resolve real regressions/review threads, then squash-merge to `develop` only when final-head gates are green and verify the integration push.
