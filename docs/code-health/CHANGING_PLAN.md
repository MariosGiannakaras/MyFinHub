...
- [ ] Migrate bounded safe adopter groups incrementally. Batch 1 (`ConfirmDialog`, `MoneyEditDialog`, reusable `CardCreateDialog` actions) merged via PR #363. Batch 2 (`DesktopUpdatePanel`, `PageErrorBoundary`, `PersistenceNotice`, `AccountMetadataSettings`) merged via PR #364. Batch 3 (`AppDateInput`, `AppSelectInput`, `CommandPalette` generic icon actions) merged via PR #365. Batch 4 (Lending generic primary/secondary/ghost/icon actions) merged via PR #366. Batch 5 (Recurring generic page-heading, mobile payment and editor primary/secondary/icon actions) merged via PR #367. Batch 6 (Loans generic page-heading and editor primary/secondary/icon actions) merged via PR #368. Batch 7 (Cards generic bank-creation heading/dialog actions) merged via PR #369. Batch 8 (`LegacyTransactionEditor` generic close/cancel/save actions) merged via PR #370. Batch 9 (Planning generic page-heading and editor primary/secondary/icon actions) merged via PR #371. Batch 10 is active and migrates only explicit category/budget/rule editor save/cancel actions in `CategoryTreeEditor` and `BudgetRuleSettings`, while retaining budget/rule row controls raw.
...
Active delivery: issue #357 / branch `chore/357-button-adopters-batch-10`.
...
- **Active Batch-10 branch/base:** `chore/357-button-adopters-batch-10`, created exactly from verified `develop@03b1c8868156f3ec38bc2d90e80292f37d295403`.
- **Current Batch-10 implementation commits:** `9cbcf31a3de22f6aafa605716b750dcf5596f4f1` migrates the explicit `CategoryTreeEditor` save action to shared primary `Button`; `dcdbca1f473b4fc74feaa1220f345d0ceccb92b7` migrates `BudgetRuleSettings` budget save plus rule-editor cancel/save to shared `Button`; `b34614cacd4d5361335dff86d222009a07d1dcc4` adds a dedicated settings ownership guard.
- **Batch-10 target controls:** **4/4 migrated** in the current implementation: category save, budget save, rule-editor cancel and rule-editor save/update. `BudgetRuleSettings` budget-delete and rule-row reorder/pause/edit/delete controls remain intentionally raw domain/row controls.
- **Current source scope:** 3/3 intended files before this plan checkpoint (`CategoryTreeEditor.tsx`, `BudgetRuleSettings.tsx`, dedicated source guard); no CSS, finance/accounting, auth, API, database, persistence, routes, Windows, release or deploy changes.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** Batch 10 still requires a draft PR, exact-head application/API/CodeQL/cross-engine/performance/Windows gates, fresh Settings/budget/rule/category rendered evidence where emitted, generated snapshot cleanup if needed, and review-thread verification before merge.
- **Intentional exceptions remain:** card-design radio options, Cards card-domain/composite controls, Lending domain/composite controls, Recurring row/menu/history controls, Loans row actions, Planning row/horizon/kind controls, BudgetRuleSettings budget/rule row controls and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** open the bounded Batch-10 draft PR, run exact-head validation, inspect fresh Settings/budget/rule/category evidence without overclaiming unavailable states, clean any generated snapshot churn, resolve real regressions/review threads, then squash-merge to `develop` only when final-head gates are green and verify the integration push.
