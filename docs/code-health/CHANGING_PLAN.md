...
- [ ] Batch 1: add TypeScript-native low-noise unused import/local diagnostics and AST-based unused-export/runtime-cycle checks with an explicit checked-in baseline; no formatter or mass reformatting.
...
- **Next action:** establish the Stage 6 low-noise hygiene baseline: TypeScript unused-local/import diagnostics plus conservative unused-export and runtime-cycle graph checks, then baseline only verified pre-existing debt.
