...
- [ ] Migrate bounded safe adopter groups incrementally. Batch 1 (`ConfirmDialog`, `MoneyEditDialog`, reusable `CardCreateDialog` actions) merged via PR #363. Batch 2 (`DesktopUpdatePanel`, `PageErrorBoundary`, `PersistenceNotice`, `AccountMetadataSettings`) merged via PR #364. Batch 3 (`AppDateInput`, `AppSelectInput`, `CommandPalette` generic icon actions) merged via PR #365. Batch 4 (Lending generic primary/secondary/ghost/icon actions) merged via PR #366. Batch 5 (Recurring generic page-heading, mobile payment and editor primary/secondary/icon actions) merged via PR #367. Batch 6 (Loans generic page-heading and editor primary/secondary/icon actions) merged via PR #368. Batch 7 is active in PR #369 and migrates only Cards generic bank-creation heading/dialog actions while preserving card-domain/composite controls as intentional raw actions.
...
Active delivery: issue #357 / PR #369 / branch `chore/357-button-adopters-batch-7`.
...
- **Active PR:** #369 — `Code health: migrate Cards shared Button adopters batch 7`.
- **Branch/base:** `chore/357-button-adopters-batch-7`, created exactly from verified `develop@db82651aac043923dbbfccf8d1fe9592454561d2`.
- **Current implementation commits:** `6fdec10fbd1d1dc712fcb40e7f75bd50a67da7a1` migrates Cards page-heading “Προσθήκη τράπεζας” plus the new-bank dialog close/cancel/submit to three shared `Button` instances and one shared `IconButton`; `820e5121d23ee3340d4da6ac7ea17a4f1ab627d7` adds a dedicated source ownership guard. Commit `b272c3892084a4480b7d67fb53ec975a5c14d433` removed an unrelated Savings change that briefly appeared on the same branch, so the net Batch-7 source scope remains Cards-only.
- **Intentional Cards raw controls:** per-bank add-card button, empty-bank target, archived-card restore/delete controls and other card/domain composite actions remain native/domain-owned because they carry dedicated card interaction contracts rather than the generic bank-creation action contract.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** PR #369 requires exact-head application/API/CodeQL/cross-engine/performance/Windows/rendered validation. Inspect fresh Cards full-page desktop/mobile evidence; because the generic new-bank dialog changes in this batch, also inspect a fresh rendered new-bank dialog state if the QA artifact emits one. Do not claim a dialog screenshot that is not present.
- **Intentional exceptions remain:** card-design radio options, Cards card-domain/composite controls, Lending domain/composite controls, Recurring row/menu/history controls, Loans row actions and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** complete PR #369 exact-head required gates, inspect fresh Cards desktop/mobile (and new-bank dialog evidence if actually emitted), remove any generated `visual-qa/**` source churn while preserving artifacts as evidence, re-run final exact-head gates if the branch moves, resolve any real review/regression, then squash-merge to `develop` only if green and verify the integration push.
