...
Active delivery: issue #357 / branch `chore/357-button-adopters-batch-14`.
...
- **Active Batch-14 branch/base:** `chore/357-button-adopters-batch-14`, created exactly from verified `develop@fa6ad460ab23dd826a5fc93339b766a54d92b172`.
- **Current Batch-14 implementation:** `24c78469a213fb4d98b696dc79387607c8a96672` migrates Savings canonical target/dialog actions to shared primitives; `e1828a38ffbacba04285ffe98b861717efab0069` adds a dedicated source guard.
- **Batch-14 target controls:** **7/7 migrated** in `SavingsPage`: **2/2 primary**, **4/4 secondary**, **1/1 icon-only** controls covering the shared target editor, desktop/mobile target-change actions, and savings dialog close/cancel/submit.
- **Preserved raw exceptions in Batch 14:** the savings-mode action cards (`Pay & Save`, `Μεταφορά στην άκρη`, `Σύνθετη αποταμίευση`) remain raw domain controls; their finance routing and source semantics are unchanged.
- **Current source scope:** **2/2 intended implementation/test files before this plan checkpoint** (`SavingsPage.tsx`, dedicated source guard); no CSS, savings/accounting semantics, auth, API, database, persistence, routes, Windows, release or deploy changes.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** Batch 14 still requires the draft PR, exact-head application/API/CodeQL/cross-engine/performance/Windows gates, fresh Savings rendered evidence where emitted, generated snapshot cleanup if needed, and review-thread verification before merge.
- **Intentional exceptions remain:** card-design radio options, Cards card-domain/composite controls, Lending domain/composite controls, Recurring row/menu/history controls, Loans row actions, Planning row/horizon/kind controls, BudgetRuleSettings budget/rule row controls, QuickAdd kind/frequent/split controls, taxonomy selectors/row controls, receipt destructive/domain controls, Credit Card deck/navigation/row/destructive/picker-specific controls, Savings mode cards, and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** open the bounded Batch-14 draft PR, run exact-head validation, inspect fresh Savings evidence without overclaiming unavailable states, clean any generated snapshot churn, resolve real regressions/review threads, then squash-merge to `develop` only when final-head gates are green and verify the integration push.
