...
Active delivery: issue #357 / branch `chore/357-button-adopters-batch-12`.
...
- **Active Batch-12 branch/base:** `chore/357-button-adopters-batch-12`, created exactly from verified `develop@6bd2ebe3eeaeab0ff91ebd69456b47a5012616b0`.
- **Batch-12 batching strategy:** consolidate more behavior-preserving generic adopter work into one validation cycle, per owner direction, while keeping the scope bounded to controls that already own the canonical `save-button` / `secondary` / `icon-button` class contracts. Domain/composite/destructive exceptions remain raw.
- **Current Batch-12 implementation commits:** `1ac9ee2067e55abe16b7a1d68dc9d1da54bfc97a` migrates `ContextualQuickAdd` close/cancel/submit; `b2bcde0af01e1c16dbf46a31ad6b394af7839541` migrates nine `CategoryIconsWorkspace` add/close/save/cancel/move editor actions; `4cf269ce8da5ae305b5f9dbc77527a61487b2da1` migrates seven `ReceiptInbox` generic close/capture/select/scan/manual/apply actions while preserving destructive/domain controls raw; `2fdedc889f14e1b4dcf3a8ddd4ca9b003b184bf8` adds the consolidated ownership/behavior source guard.
- **Batch-12 target controls:** **19/19 migrated** in the current implementation: ContextualQuickAdd **3/3**, CategoryIconsWorkspace **9/9**, ReceiptInbox **7/7**.
- **Current source scope:** **4/4 intended files before this plan checkpoint** (`ContextualQuickAdd.tsx`, `CategoryIconsWorkspace.tsx`, `ReceiptInbox.tsx`, consolidated source guard); no CSS, finance/accounting, auth, API, database, persistence, routes, Windows, release or deploy changes.
- **Preserved raw exceptions in Batch 12:** taxonomy pack/type selectors and category/subcategory row actions remain raw composites; receipt selection/open controls plus destructive delete actions remain raw; financial/OCR/taxonomy handlers and disabled/loading semantics are unchanged.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** Batch 12 still requires one draft PR and a single consolidated exact-head application/API/CodeQL/cross-engine/performance/Windows validation cycle, fresh Contextual/Settings-taxonomy/Receipt evidence where emitted, generated snapshot cleanup if needed, and review-thread verification before merge.
- **Intentional exceptions remain:** card-design radio options, Cards card-domain/composite controls, Lending domain/composite controls, Recurring row/menu/history controls, Loans row actions, Planning row/horizon/kind controls, BudgetRuleSettings budget/rule row controls, QuickAdd kind/frequent/split controls, taxonomy selectors/row controls, receipt destructive/domain controls and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** open one consolidated Batch-12 draft PR after this checkpoint, let the final branch snapshot run settle, run the required exact-head gates once, inspect fresh rendered evidence without overclaiming unavailable states, clean any generated snapshot churn if needed, resolve real regressions/review threads, then squash-merge to `develop` only when final-head gates are green and verify the integration push.
