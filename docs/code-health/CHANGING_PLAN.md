...
- [ ] Migrate bounded safe adopter groups incrementally. Batch 1 (`ConfirmDialog`, `MoneyEditDialog`, reusable `CardCreateDialog` actions) merged via PR #363. Batch 2 (`DesktopUpdatePanel`, `PageErrorBoundary`, `PersistenceNotice`, `AccountMetadataSettings`) merged via PR #364. Batch 3 (`AppDateInput`, `AppSelectInput`, `CommandPalette` generic icon actions) merged via PR #365. Batch 4 (Lending generic primary/secondary/ghost/icon actions) merged via PR #366. Batch 5 (Recurring generic page-heading, mobile payment and editor primary/secondary/icon actions) merged via PR #367. Batch 6 is active in PR #368 and migrates only Loans generic page-heading and editor primary/secondary/icon actions while preserving per-loan row controls as intentional raw domain actions.
...
Active delivery: issue #357 / PR #368 / branch `chore/357-button-adopters-batch-6`.
...
- **Active PR:** #368 — `Code health: migrate Loans shared Button adopters batch 6`.
- **Branch/base:** `chore/357-button-adopters-batch-6`, created exactly from verified `develop@8aa95989289cd3e35b3c738f5dc26f09b9c3e8d5`.
- **Current implementation commits:** `eba30cbb706b1a27c13acbe396e34ceb65a90bf3` migrates the five Loans generic actions to four shared `Button` instances plus one shared `IconButton`; `a75cd852aa85be7afeeed23490c3ad150bcb258d` adds a source-level ownership guard. Existing explicit button types, accessible names, handlers, modal-focus contract, loan/accounting semantics and compatibility class geometry remain unchanged.
- **Intentional Loans raw controls:** per-loan edit, payment/return and forgive controls remain native/domain-owned because they carry the loan-row visual and interaction contract rather than the generic shared action contract.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** PR #368 requires exact-head application/API/CodeQL/cross-engine/performance/Windows/rendered validation. Inspect fresh Loans full-page desktop/mobile plus loan-editor desktop/mobile evidence, including touch-target geometry and modal focus/close behavior.
- **Intentional exceptions remain:** card-design radio options, Lending domain/composite controls, Recurring row/menu/history controls, Loans row actions and other documented domain composites stay raw until a semantically equivalent typed API exists.
- **Next action:** complete PR #368 exact-head required gates, inspect fresh Loans desktop/mobile/editor evidence, remove any generated `visual-qa/**` source churn while preserving artifacts as evidence, re-run final exact-head gates if the branch moves, resolve any real review/regression, then squash-merge to `develop` only if green and verify the integration push.
