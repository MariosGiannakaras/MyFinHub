...
- [ ] Migrate bounded safe adopter groups incrementally. Batch 1 (`ConfirmDialog`, `MoneyEditDialog`, reusable `CardCreateDialog` actions) merged via PR #363. Batch 2 (`DesktopUpdatePanel`, `PageErrorBoundary`, `PersistenceNotice`, `AccountMetadataSettings`) merged via PR #364. Batch 3 (`AppDateInput`, `AppSelectInput`, `CommandPalette` generic icon actions) merged via PR #365. Batch 4 (Lending generic primary/secondary/ghost/icon actions) merged via PR #366. Batch 5 (Recurring generic page-heading, mobile payment and editor primary/secondary/icon actions) merged via PR #367. Batch 6 (Loans generic page-heading and editor primary/secondary/icon actions) merged via PR #368. Batch 7 (Cards generic bank-creation heading/dialog actions) merged via PR #369. Batch 8 (`LegacyTransactionEditor` generic close/cancel/save actions) merged via PR #370. Batch 9 is active and migrates only Planning generic page-heading and editor primary/secondary/icon actions while retaining Planning horizon/kind/row domain controls raw.
...
Active delivery: issue #357 / branch `chore/357-button-adopters-batch-9`.
...
- **Active Batch-9 branch/base:** `chore/357-button-adopters-batch-9`, created exactly from verified `develop@2027fdacbda7d9958392450fa5899a14f39a80ab`.
- **Current Batch-9 implementation commits:** `54b1f660daf77a849b0dd4237d947d7b1c101bb6` migrates Planning's generic page-heading action plus both editor close/cancel/primary actions to five shared `Button` and two shared `IconButton` instances while preserving explicit button types, handlers, modal focus and existing compatibility class hooks. `5a7b92ab393b38ca4561ba402e8edaa6cb730b48` adds a dedicated Planning source ownership guard.
- **Batch-9 target controls:** 7/7 generic controls migrated in the current implementation: page-heading create; draft editor close/cancel/save; completion editor close/back/submit. Planning horizon selector, scheduled-kind selector and per-scheduled-row completion/edit/skip/cancel controls remain intentionally raw domain/composite controls.
- **Stage-2 progress:** 2/5 checklist items complete; the bounded migration item remains in progress across incremental batches.
- **Validation state:** Batch 9 still requires a draft PR, exact-head application/API/CodeQL/cross-engine/performance/Windows gates, fresh rendered Planning desktop/mobile and editor evidence when emitted, generated snapshot cleanup if needed, and review-thread verification before merge.
