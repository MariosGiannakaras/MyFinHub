-- Make RLS the non-bypassable authorization boundary for the single owner.

drop policy if exists rheomiq_owner_self_select on public.rheomiq_owner;
create policy rheomiq_owner_self_select on public.rheomiq_owner for select to authenticated using (user_id = (select auth.uid()));
grant select on public.rheomiq_owner to authenticated;
...
as $$ select exists (select 1 from public.rheomiq_owner o where o.singleton is true and o.user_id = (select auth.uid())); $$;
...
create policy rheomiq_state_owner_select on public.rheomiq_app_state for select to authenticated using ((select public.rheomiq_is_owner()));
create policy rheomiq_state_owner_insert on public.rheomiq_app_state for insert to authenticated with check ((select public.rheomiq_is_owner()));
create policy rheomiq_state_owner_update on public.rheomiq_app_state for update to authenticated using ((select public.rheomiq_is_owner())) with check ((select public.rheomiq_is_owner()));
grant select, insert, update on public.rheomiq_app_state to authenticated;
...
create policy rheomiq_backups_owner_select on public.rheomiq_backups for select to authenticated using ((select public.rheomiq_is_owner()));
create policy rheomiq_backups_owner_insert on public.rheomiq_backups for insert to authenticated with check ((select public.rheomiq_is_owner()));
create policy rheomiq_backups_owner_delete on public.rheomiq_backups for delete to authenticated using ((select public.rheomiq_is_owner()));
grant select, insert, delete on public.rheomiq_backups to authenticated;
grant usage, select on sequence public.rheomiq_backups_id_seq to authenticated;
...
with check ((select public.rheomiq_is_owner()) and actor_user_id = (select auth.uid()));
grant insert on public.rheomiq_audit_log to authenticated;
grant usage, select on sequence public.rheomiq_audit_log_id_seq to authenticated;
...
as $$
begin
  if coalesce(auth.role(), '') <> 'service_role' and not public.rheomiq_is_owner() then raise exception using errcode = '42501', message = 'FORBIDDEN'; end if;
...
as $$
  delete from public.rheomiq_backups where id in (
    select id from public.rheomiq_backups order by created_at desc, id desc offset 100
  );
$$;
...
revoke all on function public.rheomiq_prune_backups() from public, anon;
...
grant execute on function public.rheomiq_prune_backups() to authenticated, service_role;
