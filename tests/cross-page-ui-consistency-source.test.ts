...
describe('cross-page UI consistency contracts',()=>{
  ...
  it('reuses existing shared controls in card creation without adding eager global CSS',()=>{
    ...
    expect(cardDialog).toContain("import { Button } from './Button'");
    expect(cardDialog).toContain("import { IconButton } from './IconButton'");
    expect(cardDialog).toContain('<IconButton className="close-picker" label="Κλείσιμο"');
    ...
    // CardsPage is migrated separately; until then its approved compatibility hooks remain explicit.
