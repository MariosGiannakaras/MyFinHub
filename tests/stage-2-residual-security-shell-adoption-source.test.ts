...
describe('Stage 2 residual security and shell action adoption',()=>{
  it('uses shared primitives for the global Quick Add and history close while preserving shell semantics',()=>{
    ...
    expect(shell).toContain('<Button type="button" variant="primary" className="primary-action" data-global-quick-entry="desktop"');
    ...
    expect(shell).not.toContain('<button type="button" className="primary-action"');
  ...
  it('uses shared primary Button for login submit without changing submit or disabled behavior',()=>{
    ...
    expect(login).toContain('<Button variant="primary" className="primary-action login-submit" type="submit" disabled={busy||!ready}');
    expect(login).toContain('onSubmit={submit}');
    expect(login).toContain('if(busy||!ready)return');
    expect(login).not.toContain('<button className="primary-action login-submit"');
  ...
  it('uses shared primary Button for MFA enrollment and verification while preserving auth guards',()=>{
    ...
    expect(mfa).toContain('<Button variant="primary" className="primary-action login-submit" type="button" disabled={busy} aria-busy={busy} onClick={startEnrollment}>');
    expect(mfa).toContain('<Button variant="primary" className="primary-action login-submit" type="submit" disabled={busy || code.length !== 6}');
    ...
    expect(mfa).not.toContain('<button className="primary-action login-submit"');
  });
});
