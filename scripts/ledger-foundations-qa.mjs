...
try{
  ...
  let splitState=await c.call("function(){const row=document.querySelector('[data-transaction-kind=split]');return {exists:!!row,parts:row?.querySelectorAll('.transaction-split-part').length||0,text:row?.textContent||'',amount:row?.querySelector('.amount')?.textContent||''}}");assert(splitState.exists,'split parent row exists');assert(splitState.parts===2,'split parent exposes exactly two detail portions');assert(splitState.amount.includes('100,00')&&splitState.amount.includes('−')&&splitState.text.includes('Σούπερ μάρκετ')&&splitState.text.includes('Σπίτι'),'split row shows one derived expense amount and both portions');
  ...
  await setAriaInput('Ποσό μέρους 2','20');const editedTotal=await c.call("function(){return document.querySelector('.split-head [aria-live=polite]')?.textContent||''}");assert(editedTotal.includes('90,00'),'split edit derives the new parent total live');await clickText('.quick-modal footer button','Εφαρμογή αλλαγών');await waitFor("function(){return !document.querySelector('.quick-modal')}",'split corrected close');splitState=await c.call("function(){const row=document.querySelector('[data-transaction-kind=split]');return {amount:row?.querySelector('.amount')?.textContent||'',parts:row?.querySelectorAll('.transaction-split-part').length||0}}");assert(splitState.amount.includes('90,00')&&splitState.parts===2,'saved split amount follows edited parts exactly');
