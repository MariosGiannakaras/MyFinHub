...
try{
  ...
  console.log('Ledger QA: create split transaction with live allocation');
  await navigate('Συναλλαγές','Συναλλαγές');await clickText('button.primary-action','Γρήγορη προσθήκη');await waitFor("function(){return !!document.querySelector('.quick-modal')}",'Quick Add split');await clickText('.generic-kind-grid button','Σύνθετη αγορά');await setLabelInput('Ποσό','100');
  ...
  const allocationText=await c.call("function(){return document.querySelector('.split-head [aria-live=polite]')?.textContent||''}");assert(allocationText.includes('Συμπληρώθηκαν'),'split editor reports exact allocation before save');
  ...
  const splitState=await c.call("function(){const row=document.querySelector('[data-transaction-kind=split]');return {exists:!!row,parts:row?.querySelectorAll('.transaction-split-part').length||0,text:row?.textContent||'',amount:row?.querySelector('.amount')?.textContent||''}}");assert(splitState.exists,'split parent row exists');assert(splitState.parts===2,'split parent exposes exactly two detail portions');assert(splitState.amount.includes('−')&&splitState.text.includes('Σούπερ μάρκετ')&&splitState.text.includes('Σπίτι'),'split row shows one expense amount and both portions');

  console.log('Ledger QA: reject incomplete split and preserve modal state');
  await c.call("function(){document.querySelector('[data-transaction-kind=split] button[aria-label^=\"Επεξεργασία\"]')?.click()}");await waitFor("function(){return !!document.querySelector('.quick-modal')}",'split edit');await setAriaInput('Ποσό μέρους 2','20');const remaining=await c.call("function(){return document.querySelector('.split-head [aria-live=polite]')?.textContent||''}");assert(remaining.includes('Απομένουν'),'split edit reports remaining allocation');await clickText('.quick-modal footer button','Εφαρμογή αλλαγών');const splitError=await c.call("function(){return document.querySelector('.quick-modal .form-error')?.textContent||''}");assert(splitError.includes('Απομένουν'),'incomplete split is rejected with direct feedback');
  await setAriaInput('Ποσό μέρους 2','30');await clickText('.quick-modal footer button','Εφαρμογή αλλαγών');await waitFor("function(){return !document.querySelector('.quick-modal')}",'split corrected close');

  console.log('Ledger QA: delete and undo split atomically');
  await c.call("function(){window.confirm=()=>true;document.querySelector('[data-transaction-kind=split] button[aria-label^=\"Διαγραφή\"]')?.click()}");await sleep(160);let splitExists=await c.call("function(){return !!document.querySelector('[data-transaction-kind=split]')}");assert(!splitExists,'split parent is deleted in one action');await c.call("function(){document.querySelector('button[aria-label=\"Αναίρεση τελευταίας αλλαγής\"]')?.click()}");await sleep(180);splitExists=await c.call("function(){return !!document.querySelector('[data-transaction-kind=split]')}");assert(splitExists,'undo restores the entire split parent and portions');
  ...
  await c.send('Emulation.setDeviceMetricsOverride',{width:375,height:812,deviceScaleFactor:1,mobile:true});await sleep(180);const mobile=await c.call("function(){return {transfer:!!document.querySelector('[data-mobile-transaction-kind=transfer]'),split:!!document.querySelector('[data-mobile-transaction-kind=split]'),parts:document.querySelector('[data-mobile-transaction-kind=split]')?.querySelectorAll('.transaction-split-part').length||0,overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+2}}");assert(mobile.transfer&&mobile.split&&mobile.parts===2,'mobile transaction list preserves transfer/split detail');assert(!mobile.overflow,'ledger transaction mobile view has no horizontal overflow');const mobileShot=await c.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});writeFileSync(`${evidenceDir}/ledger-foundations-mobile.png`,Buffer.from(mobileShot.data,'base64'));
