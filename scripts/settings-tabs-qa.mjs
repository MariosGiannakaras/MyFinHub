...
try{
  ...
  for(const mode of [{name:'desktop',width:1440,height:1000},{name:'mobile',width:375,height:812}]){
    ...
    for(const tab of tabs){
      ...
      if(tab.id==='profile'){
        const profile=await c.call("function(){const root=document.querySelector('.account-security-settings');const buttons=[...(root?.querySelectorAll('button')||[])];return {text:root?.textContent||'',inputs:root?.querySelectorAll('input').length||0,emailAction:buttons.some(button=>(button.textContent||'').includes('Αλλαγή email')&&!button.disabled),passwordAction:buttons.some(button=>(button.textContent||'').includes('Αλλαγή κωδικού')&&!button.disabled),pinDisabled:buttons.some(button=>(button.textContent||'').includes('Ενεργοποίηση PIN')&&button.disabled),devicePanel:Boolean(root?.querySelector('.device-access-settings')),pinDots:root?.querySelectorAll('.account-security-pin-dots i').length||0}}");assert(profile.text.includes('Πρόσβαση & ασφάλεια'),'User access tab has distinct access identity');assert(profile.text.includes('Αλλαγή email'),'User access exposes real email change');assert(profile.text.includes('Αλλαγή κωδικού'),'User access exposes real password change');assert(profile.text.includes('PIN εφαρμογής'),'User access exposes app PIN controls');assert(profile.text.includes('4 ψηφίων'),'User access uses a four-digit PIN');assert(profile.text.includes('Συνδεδεμένες συσκευές'),'User access exposes connected devices');assert(profile.inputs>=6,'User access renders account-security inputs');assert(profile.emailAction&&profile.passwordAction,'User access auth actions are actionable');assert(profile.pinDisabled,'Browser QA truthfully keeps Windows-only PIN mutation disabled');assert(profile.devicePanel,'Connected-device panel is rendered');assert(profile.pinDots>=8,'PIN setup renders four-digit visual indicators');
      }
