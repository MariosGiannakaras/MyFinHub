...
try{
  ...
  const openMobileMore=async()=>{const expanded=await c.eval(`document.querySelector('button[aria-label="Περισσότερες ενότητες"]')?.getAttribute('aria-expanded')==='true'`);if(!expanded)await clickLabel('Περισσότερες ενότητες');await waitFor(`document.querySelector('button[aria-label="Περισσότερες ενότητες"]')?.getAttribute('aria-expanded')==='true'&&Boolean(document.querySelector('[aria-labelledby="mobile-more-title"]'))`,'mobile More dialog');await sleep(220)};
  ...
  for(const width of [320,430]){
    ...
    await openMobileMore();assert(await c.eval(`document.querySelector('[aria-labelledby="mobile-more-title"]').contains(document.activeElement)`),`mobile More focus ${width}`);assert(await c.eval(`Number(getComputedStyle(document.querySelector('.mobile-more-backdrop')).zIndex)>Number(getComputedStyle(document.querySelector('.mobile-nav')).zIndex)`),`mobile modal backdrop covers nav ${width}`);assert(await c.eval(`(()=>{const menuBottom=parseFloat(getComputedStyle(document.querySelector('.mobile-more-menu')).bottom),navHeight=document.querySelector('.mobile-nav').getBoundingClientRect().height;return menuBottom>=navHeight+6})()`),`mobile More reserves nav spacing ${width}`);await clickText('.mobile-more-menu button','Πάγια');await waitHeading('Πάγια & Συνδρομές');
