...
try{
  ...
  const clickAria=async label=>{const clicked=await c.eval(`(()=>{const node=[...document.querySelectorAll('button[aria-label]')].find(item=>item.getAttribute('aria-label')===${q(label)});if(!node)return false;node.click();return true})()`);assert(clicked,`missing aria button ${label}`)};
  const openMore=async()=>{const open=await c.eval(`document.querySelector('button[aria-label="Περισσότερες ενότητες"]')?.getAttribute('aria-expanded')==='true'`);if(!open)await clickAria('Περισσότερες ενότητες');await waitFor(`Boolean(document.querySelector('[aria-labelledby="mobile-more-title"]'))`,'mobile More sheet')};
  ...
  await clickText('button','Γρήγορη προσθήκη');await waitFor(`Boolean(document.querySelector('[aria-labelledby="quick-add-title"]'))`,'Quick Add');assert(await c.eval(`document.querySelector('[aria-labelledby="quick-add-title"]').contains(document.activeElement)`),'Quick Add focus');await clickAria('Κλείσιμο καταχώρισης');
