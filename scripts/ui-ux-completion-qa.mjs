...
try{
  ...
  const setSort=async value=>{assert(await c.call("function(value){const select=document.querySelector('.loan-sort-controls select');if(!select)return false;const setter=Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set;setter.call(select,value);select.dispatchEvent(new Event('change',{bubbles:true}));return true}",[value]),`set loan sort ${value}`);await sleep(80)};
  await setSort('name');await clickText('.sort-direction-control button','ASC');assert((await c.eval("document.querySelector('.loan-list-row h3')?.textContent"))==='Laptop','text ASC ordering');await clickText('.sort-direction-control button','DESC');assert((await c.eval("document.querySelector('.loan-list-row h3')?.textContent"))==='QA Long Loan','text DESC ordering');
  await setSort('amount');await clickText('.sort-direction-control button','ASC');assert((await c.eval("document.querySelector('.loan-list-row h3')?.textContent"))==='Laptop','numeric ASC ordering');await clickText('.sort-direction-control button','DESC');assert((await c.eval("document.querySelector('.loan-list-row h3')?.textContent"))==='QA Long Loan','numeric DESC ordering');
  await setSort('next');await clickText('.sort-direction-control button','ASC');const ascIds=await c.eval("[...document.querySelectorAll('.loan-list-row')].map(row=>row.querySelector('h3')?.textContent)");await clickText('.sort-direction-control button','DESC');const descIds=await c.eval("[...document.querySelectorAll('.loan-list-row')].map(row=>row.querySelector('h3')?.textContent)");assert(ascIds.length===descIds.length&&ascIds.length>=2,'null-date sort retains all rows');assert(new Set(ascIds).size===ascIds.length&&new Set(descIds).size===descIds.length,'stable tie-break keeps unique deterministic rows');

  console.log('Completion QA: form errors are associated with dialogs and fields');
  await navigate({page:'savings'},'Αποταμίευση');await clickText('button','Μεταφορά στην άκρη');await clickText('.editor-actions button','Καταχώριση αποταμίευσης');await waitFor("function(){return Boolean(document.querySelector('.savings-dialog .form-error'))}",'savings error');const savingAssociation=await c.eval("(()=>{const dialog=document.querySelector('.savings-dialog'),error=dialog?.querySelector('.form-error'),input=dialog?.querySelector('input[data-autofocus]');return Boolean(error?.id&&dialog?.getAttribute('aria-describedby')?.split(/\\s+/).includes(error.id)&&input?.getAttribute('aria-describedby')?.split(/\\s+/).includes(error.id)&&input?.getAttribute('aria-invalid')==='true')})()");assert(savingAssociation,'savings error association');
