...
try{
  ...
  const clickAria=async label=>{const ok=await c.call("function(label){const node=document.querySelector('button[aria-label=\"'+CSS.escape(label)+'\"]');if(!node)return false;node.click();return true}",[label]);assert(ok,`missing ${label}`)};
