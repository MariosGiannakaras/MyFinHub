...
try{
  ...
  const shellState=await c.eval(`(()=>{const saved=document.querySelector('.file-panel[data-save-state="saved"]');const next=document.querySelector('.period-control button[aria-label^="Επόμενος μήνας"]');const pending=document.querySelector('[data-dashboard-section="pending"]');const visible=node=>{const rect=node.getBoundingClientRect(),style=getComputedStyle(node);return rect.width>0&&rect.height>0&&style.display!=='none'&&style.visibility!=='hidden'};const pendingPanels=pending?[...pending.children].filter(node=>node.matches('article.panel')&&visible(node)).length:0;return {savedQuiet:saved?.classList.contains('is-quiet')||false,savedSmallDisplay:saved?getComputedStyle(saved.querySelector('small')).display:'',savedLive:saved?.getAttribute('aria-live'),nextDisabled:next?.disabled||false,periodText:document.querySelector('.period-control>span')?.textContent||'',pendingPanels}})()`);
  ...
  assert(shellState.pendingPanels===3,'desktop pending obligations expose exactly three visible primary panels');
