...
try{
  ...
  const navigate=async params=>{await c.send('Page.navigate',{url:urlFor(params)});await waitFor("function(){return document.querySelector('#main-workspace h1')?.textContent==='Οι λογαριασμοί μου'}",'Dashboard heading');await sleep(100)};
  ...
  await viewport(1280,900,false);await navigate({page:'dashboard',motion:'reduced'});
  let state=await c.eval(`(()=>{const visible=node=>{if(!node)return false;const rect=node.getBoundingClientRect();const style=getComputedStyle(node);return rect.width>0&&rect.height>0&&style.display!=='none'&&style.visibility!=='hidden'};const globals=[...document.querySelectorAll('[data-global-quick-entry]')].filter(visible);const saved=document.querySelector('.file-panel[data-save-state="saved"]');const next=document.querySelector('.period-control button[aria-label^="Επόμενος μήνας"]');const pending=document.querySelector('[data-dashboard-section="pending"]');return {globals:globals.map(node=>node.getAttribute('data-global-quick-entry')),headingGeneric:!!document.querySelector('.page-heading .save-button'),quickGeneric:!!document.querySelector('[data-dashboard-section="quick-entry"]>.wide-action'),prefilled:document.querySelectorAll('[data-prefilled-quick-entry]').length,accounts:document.querySelectorAll('[data-account-quick-entry]').length,quickTitle:document.querySelector('[data-dashboard-section="quick-entry"] .panel-head span')?.textContent||'',savedQuiet:saved?.classList.contains('is-quiet')||false,savedSmallDisplay:saved?getComputedStyle(saved.querySelector('small')).display:'',savedLive:saved?.getAttribute('aria-live'),nextDisabled:next?.disabled||false,month:document.querySelector('.period-control>span')?.textContent||'',pendingColumns:pending?getComputedStyle(pending).gridTemplateColumns.split(/\s+/).filter(Boolean).length:0,overflow:Math.max(document.documentElement.scrollWidth,document.body.scrollWidth)-innerWidth}})()`);
  ...
  assert(state.savedQuiet&&state.savedSmallDisplay==='none'&&state.savedLive===null,'healthy saved chrome is quiet and non-announcing');
  assert(state.nextDisabled&&state.month.includes('Αύγουστος')&&state.month.includes('2026'),'current reporting month disables future navigation');
  assert(state.pendingColumns===3,'desktop pending obligations use a compact three-column hierarchy');
  ...
  await viewport(375,812,true);await navigate({page:'dashboard',motion:'reduced'});
  state=await c.eval(`(()=>{const visible=node=>{if(!node)return false;const rect=node.getBoundingClientRect();const style=getComputedStyle(node);return rect.width>0&&rect.height>0&&style.display!=='none'&&style.visibility!=='hidden'};const globals=[...document.querySelectorAll('[data-global-quick-entry]')].filter(visible);return {globals:globals.map(node=>node.getAttribute('data-global-quick-entry')),headingGeneric:!!document.querySelector('.page-heading .save-button'),quickGeneric:!!document.querySelector('[data-dashboard-section="quick-entry"]>.wide-action'),prefilled:document.querySelectorAll('[data-prefilled-quick-entry]').length,accounts:document.querySelectorAll('[data-account-quick-entry]').length,overflow:Math.max(document.documentElement.scrollWidth,document.body.scrollWidth)-innerWidth}})()`);
