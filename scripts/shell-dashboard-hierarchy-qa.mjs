...
try{
  ...
  const shellState=await c.eval(`(()=>{const saved=document.querySelector('.file-panel[data-save-state="saved"]');const next=document.querySelector('.period-control button[aria-label^="Επόμενος μήνας"]');const pending=document.querySelector('[data-dashboard-section="pending"]');const visible=node=>{const rect=node.getBoundingClientRect(),style=getComputedStyle(node);return rect.width>0&&rect.height>0&&style.display!=='none'&&style.visibility!=='hidden'};const cards=pending?[...pending.querySelectorAll(':scope > article.panel')].filter(visible):[];const pendingRect=pending?.getBoundingClientRect();const widths=cards.map(node=>node.getBoundingClientRect().width);const pendingThreeColumn=Boolean(pending&&pendingRect&&getComputedStyle(pending).display==='grid'&&cards.length===3&&widths.every(width=>width>0&&width<=pendingRect.width*.4));return {savedQuiet:saved?.classList.contains('is-quiet')||false,savedSmallDisplay:saved?getComputedStyle(saved.querySelector('small')).display:'',savedLive:saved?.getAttribute('aria-live'),nextDisabled:next?.disabled||false,periodText:document.querySelector('.period-control>span')?.textContent||'',pendingThreeColumn}})()`);
  ...
  assert(shellState.pendingThreeColumn,'desktop pending obligations use a compact three-column hierarchy');
