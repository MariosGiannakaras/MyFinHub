...
try{
  ...
  const collapsedCompletion=await c.call("function(){const history=document.querySelector('[data-loan-history]');return {open:Boolean(history?.open),count:(history?.querySelector('summary strong')?.textContent||''),hasLaptop:[...document.querySelectorAll('[data-loan-lifecycle=completed]')].some(row=>(row.textContent||'').includes('Laptop'))}}");
  assert(!collapsedCompletion.open,'completed loan history stays collapsed after completion');assert(collapsedCompletion.hasLaptop,'completed loan is retained in historical disclosure');assert(Number(collapsedCompletion.count)>=1,'completed history count increments');
  ...
  const inactiveCollapsed=await c.call("function(){const history=document.querySelector('[data-inactive-recurring-history]');return {open:Boolean(history?.open),hasCloud:[...history?.querySelectorAll('[data-recurring-status]')||[]].some(row=>(row.textContent||'').includes('Cloud'))}}");assert(!inactiveCollapsed.open,'inactive recurring history stays collapsed after pause');assert(inactiveCollapsed.hasCloud,'paused item is retained in inactive history');await shot('recurring-inactive-collapsed-desktop');
