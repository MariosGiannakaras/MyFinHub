...
try{
  ...
  await c.eval("document.querySelector('.owned-date-popover button[aria-label=\"Κλείσιμο ημερολογίου\"]')?.click()");await waitFor("function(){return !document.querySelector('.owned-date-popover')}",'calendar close');
  await waitFor("function(){return document.activeElement===document.querySelector('.quick-modal .owned-date-shell input')}",'calendar focus restoration');
