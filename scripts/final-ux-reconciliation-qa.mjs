...
try{
 ...
 state=await c.call("function(){const dialog=document.querySelector('[aria-labelledby=\"change-history-title\"]');const rect=dialog?.getBoundingClientRect();const actions=[...document.querySelectorAll('.history-actions button')].map(node=>node.getBoundingClientRect().height);return {width:rect?.width||0,right:rect?.right||0,viewportWidth:innerWidth,actions,overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+1}}");
 assert(state.width>0&&state.right<=state.viewportWidth+1,'mobile History dialog stays inside viewport');assert(state.actions.every(height=>height>=44),'mobile History undo/redo targets remain >=44px');assert(!state.overflow,'mobile History introduces no horizontal overflow');await shot('final-ux-history-mobile');
