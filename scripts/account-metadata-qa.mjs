...
try{
  ...
  const openPayrollEditor=async()=>{const opened=await c.call("function(){const row=[...document.querySelectorAll('.account-management-row')].find(item=>(item.textContent||'').includes('Μισθοδοσία'));const button=row?.querySelector('.account-management-edit');if(!button)return false;button.click();return true}");assert(opened,'payroll Account Management editor opens');await waitFor("function(){return Boolean(document.querySelector('.account-management-modal.is-edit'))}",'payroll Account Management edit modal')};
