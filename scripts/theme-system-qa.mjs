...
try{
  ...
  const switched=await c.call(`async function(){const group=[...document.querySelectorAll('[role="radiogroup"]')].find(node=>node.getAttribute('aria-label')==='Θέμα εμφάνισης');const target=[...group.querySelectorAll('button')].find(button=>button.innerText.includes('Light'));target.click();await new Promise(resolve=>setTimeout(resolve,80));return {theme:document.documentElement.dataset.theme,stored:localStorage.getItem('myfinhub.theme'),heading:document.querySelector('#main-workspace h1')?.textContent||'',selected:target.getAttribute('aria-checked')}}`);
  assert(switched.theme==='light'&&switched.stored==='light','Settings Light choice applies and persists locally');assert(switched.heading.includes('Ρυθμίσεις'),'theme switch does not navigate away');assert(switched.selected==='true','theme radio semantics update after switching');await shot('light-settings-selected-desktop');
