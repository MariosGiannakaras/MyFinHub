...
try{
  ...
  const switched=await c.call(`async function(){const before={theme:document.documentElement.dataset.theme,pref:document.documentElement.dataset.themePreference,stored:localStorage.getItem('myfinhub.theme')};const group=[...document.querySelectorAll('[role="radiogroup"]')].find(node=>node.getAttribute('aria-label')==='Θέμα εμφάνισης');const target=[...group.querySelectorAll('button')].find(button=>button.querySelector('b')?.innerText.trim()==='Light');const disabled=target.disabled;target.click();await new Promise(resolve=>setTimeout(resolve,160));return {before,disabled,theme:document.documentElement.dataset.theme,pref:document.documentElement.dataset.themePreference,stored:localStorage.getItem('myfinhub.theme'),heading:document.querySelector('#main-workspace h1')?.textContent||'',selected:target.getAttribute('aria-checked')}}`);
  assert(switched.theme==='light'&&switched.pref==='light'&&switched.stored==='light',`Settings Light choice applies and persists locally: ${JSON.stringify(switched)}`);assert(switched.heading.includes('Ρυθμίσεις'),'theme switch does not navigate away');assert(switched.selected==='true','theme radio semantics update after switching');await shot('light-settings-selected-desktop');
  ...
  await c.send('Page.navigate',{url:baseUrl});await waitFor("function(){return document.readyState==='complete'&&!!document.querySelector('#main-workspace')}",'QA workspace restored');await applyTheme('dark');await c.call("function(){document.querySelector('[data-global-quick-entry=\"desktop\"]')?.click();return true}");await waitFor("function(){return !!document.querySelector('.quick-modal')}",'Quick Entry modal');await c.call("function(){const input=document.querySelector('.quick-modal input:not([type=hidden])');input?.focus();return true}");await shot('dark-quick-entry-focus-desktop');
