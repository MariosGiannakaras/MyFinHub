...
try{
  ...
  const openMore=async()=>{const open=await c.eval(`document.querySelector('button[aria-label="Περισσότερες ενότητες"]')?.getAttribute('aria-expanded')==='true'`);if(!open)await clickText('.mobile-nav button','Περισσότερα');await waitFor(`Boolean(document.querySelector('[aria-labelledby="mobile-more-title"]'))`,'More sheet')};
  ...
  await clickText('.sidebar nav button','Ρυθμίσεις');await waitHeading('Ρυθμίσεις');assert(await c.eval(`getComputedStyle(document.querySelector('.settings-draft-actions')).position!=='sticky'`),'desktop settings actions remain non-sticky');
  ...
  await clickText('.mobile-nav button','Κάρτες');await waitHeading('Κάρτες');await noOverflow('Cards 375');assert(await c.eval(`getComputedStyle(document.querySelector('.banks-scroll')).overflowX==='auto'||getComputedStyle(document.querySelector('.banks-scroll')).overflowX==='scroll'`),'Cards banks stay in contained horizontal rail');assert(await c.eval(`document.querySelector('.payment-card').getBoundingClientRect().width<=window.innerWidth-24`),'payment card fits phone viewport');
  ...
  await morePage('Ρυθμίσεις','Ρυθμίσεις');await noOverflow('Settings 375');assert(await fullWidthStack('.settings-grid','.settings-grid>article',2),'Settings one-column groups');assert(await c.eval(`getComputedStyle(document.querySelector('.settings-draft-actions')).position==='sticky'`),'Settings actions sticky on phone');
