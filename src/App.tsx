...
import type { AttentionDecision, CardBank, EventKind, FinanceData, FinanceEvent, Loan, PaymentCard, RecurringItem, ReviewDecision, ScheduledTransaction } from './types';
...
function FinanceApp({userEmail,onLogout}:{userEmail:string|null;onLogout:()=>void}){
  ...
  const openGeneric=(kind:EventKind='expense',prefill:QuickPrefill|null=null)=>{setEditingEventId(null);setQuickContext({token:quickToken(),mode:'generic',kind,prefill});setQuickOpen(true)};
  const openSpecial=(context:Omit<Exclude<QuickActionContext,{mode:'generic'}>,'token'>)=>{setEditingEventId(null);setQuickContext({...context,token:quickToken()} as QuickActionContext);setQuickOpen(true)};
  useEffect(()=>{const onKey=(e:KeyboardEvent)=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){if(!GENERIC_ENTRY_PAGES.has(page))return;e.preventDefault();if(quickOpen)return;openGeneric('expense')}};addEventListener('keydown',onKey);return()=>removeEventListener('keydown',onKey)},[quickOpen,page]);
  ...
  const editEvent=(id:string)=>{const event=(data.state.events??[]).find(item=>item.id===id);setEditingEventId(id);setQuickContext({token:quickToken(),mode:'generic',kind:event?.kind||'expense',prefill:null});setQuickOpen(true)};
  ...
  const handleAttention=(item:AttentionItem)=>{
    ...
    if(item.action==='pay_credit'&&item.cardId){openSpecial({mode:'credit',action:'payment',cardId:item.cardId,amount:item.amount,fromAccountId:data.state.settings.defaultExpenseAccount});return}
    ...
    if(item.action==='complete_scheduled'){navigate('planning');return}
    navigate('planning');
  };
  ...
  const content=page==='dashboard'?<DashboardPage data={data} month={month} asOf={today} motionMode="full" onQuickAdd={(prefill?:QuickPrefill)=>openGeneric('expense',prefill||null)} onAccountQuickAdd={(accountId,kind)=>kind==='savings'?openGeneric('transfer',null):openGeneric('expense',{note:'',amount:0,accountId})} onTransactions={()=>navigate('transactions')} onPlanning={()=>navigate('planning')} onAttention={()=>navigate('attention')}/>
