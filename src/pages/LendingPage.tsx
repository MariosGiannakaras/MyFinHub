...
export function LendingPage({data,asOf,onCreateEvent,onQuickAdd}:{data:FinanceData;asOf:string;onCreateEvent:(event:FinanceEvent)=>void;onQuickAdd?:(context:LendingQuickContext)=>void}){
  ...
  return <div className="page-stack lending-page">
    ...
    <section className="page-heading lending-mobile-only"><div><span className="eyebrow">ΔΑΝΕΙΚΑ / ΟΦΕΙΛΕΣ</span><h1>Δανεικά / Οφειλές</h1><p>Πλήρες ιστορικό για όσα πλήρωσες για άλλους και όσα σου επέστρεψαν. Δεν θεωρούνται προσωπικό έξοδο όσο παραμένουν απαίτηση.</p></div><div className="heading-actions"><Button variant="secondary" type="button" className="privacy-toggle" aria-pressed={visible} onClick={()=>setVisible(value=>!value)}>{visible?<EyeOff size={17}/>:<Eye size={17}/>} {visible?'Απόκρυψη στοιχείων':'Εμφάνιση στοιχείων'}</Button><Button variant="primary" type="button" onClick={()=>start('lending')}><HandCoins size={17}/> Νέα κίνηση</Button></div></section>
    ...
    <DialogShell open={open} presentation="editor" className="lending-dialog" ariaLabelledBy="lending-dialog-title" ariaDescribedBy={error?'lending-dialog-error':undefined} preferredFocus='[data-autofocus="true"]' onRequestClose={close}><div className="panel-head"><div><span id="lending-dialog-title">{kind==='lending'?'Πλήρωσα για άλλον':'Μου επέστρεψαν'}</span><small>{kind==='lending'?'Αυξάνει όσα σου χρωστά το πρόσωπο. Η ημερομηνία επιστροφής είναι προαιρετική και χρησιμοποιείται μόνο για υπενθυμίσεις.':'Μειώνει την οφειλή και αυξάνει τον επιλεγμένο λογαριασμό.'}</small></div><IconButton type="button" aria-label="Κλείσιμο κίνησης δανεικών" onClick={close}><X/></IconButton></div><div className="settings-form editor-grid"><div className="lending-person-field"><label><span>Πρόσωπο</span><AppTextInput data-autofocus="true" value={person} onChange={e=>setPerson(e.target.value)} placeholder="Όνομα" aria-autocomplete="list" aria-controls={knownPeople.length?'known-people-options':undefined} invalid={Boolean(error)} aria-describedby={error?'lending-dialog-error':undefined}/></label>{knownPeople.length?<div id="known-people-options" className="known-people-suggestions" role="listbox" aria-label="Γνωστά πρόσωπα">{knownPeople.map(row=><button type="button" role="option" aria-selected={row.person===person} key={row.person} onClick={()=>setPerson(row.person)}>{row.person}{kind==='repayment'?<small>{money.format(row.outstanding)}</small>:null}</button>)}</div>:null}</div><label><span>Ποσό</span><MoneyInput value={amount} onValueChange={setAmount} placeholder="0,00" invalid={Boolean(error)} aria-describedby={error?'lending-dialog-error':undefined}/></label><label><span>Ημερομηνία</span><AppDateInput value={date} onChange={e=>{setDate(e.target.value);if(expectedReturnDate&&expectedReturnDate<e.target.value)setExpectedReturnDate('')}}/></label><label><span>{kind==='lending'?'Πληρωμή από':'Επιστροφή σε'}</span><AppSelectInput value={account} onChange={e=>setAccount(e.target.value)}>{accounts.map(item=><option key={item.id} value={item.id}>{accountDisplayName(data,item.id)}</option>)}</AppSelectInput></label>{kind==='lending'?<label><span>Αναμενόμενη επιστροφή <em>προαιρετικό</em></span><AppDateInput value={expectedReturnDate} min={date} onChange={e=>setExpectedReturnDate(e.target.value)}/></label>:null}<label className="wide"><span>Σχόλιο <em>προαιρετικό</em></span><AppTextInput value={note} onChange={e=>setNote(e.target.value)}/></label></div>{error?<FormError id="lending-dialog-error">{error}</FormError>:null}<div className="editor-actions"><Button variant="secondary" type="button" onClick={close}>Ακύρωση</Button><Button variant="primary" type="button" onClick={submit}>Καταχώριση</Button></div></DialogShell>
  </div>;
}
