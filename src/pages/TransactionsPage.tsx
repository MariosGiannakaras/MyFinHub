...
export function TransactionsPage({
  ...
  const trendText=(value:number|null,betterWhenLower=false)=>value===null?'— έναντι προηγ. μήνα':`${value>0?'↑':value<0?'↓':'→'} ${Math.abs(value)}% από προηγ. μήνα`;
  ...
  const metadata=(row:TransactionRow)=>{const label=categoryLabel(row);if(row.source!=='legacy')return label;return `${label} · ${row.overridden?'Ιστορικό · override':'Ιστορικό'}`};
  ...
  const mobileRowActions=(row:TransactionRow,title:string)=><div className="mobile-row-actions"><button type="button" className="text-button" style={{minHeight:44}} aria-label={`Επεξεργασία ${title}`} onClick={()=>edit(row)}><Pencil size={15}/> Επεξεργασία</button><button type="button" className="text-button danger-text" style={{minHeight:44}} aria-label={`Διαγραφή ${title}`} onClick={()=>askDelete(row)}><Trash2 size={15}/> Διαγραφή</button></div>;

  return <div className="page-stack transactions-approved">
    <section className="page-heading transactions-approved-heading"><div><span className="eyebrow">ΣΥΝΑΛΛΑΓΕΣ</span><h1>Οι συναλλαγές μου</h1><p>Δείτε και διαχειριστείτε όλες τις συναλλαγές σας. Αναζήτηση, φίλτρα, κατηγορίες και πλήρης έλεγχος.</p></div></section>
    ...
    <section className="transactions-approved-summary desktop-finance-table" aria-label="Σύνοψη συναλλαγών μήνα">
      ...
      <article className="transactions-summary-card expense"><span className="transactions-summary-icon"><TrendingDown size={24}/></span><div className="transactions-summary-copy"><small>Σύνολο εξόδων</small><strong className="negative">−{money.format(flow.expense)}</strong><span className={trendClass(expenseDelta,true)}>{trendText(expenseDelta,true)}</span></div></article>
    ...
    <section className="transactions-approved-shell">
      <div className="transactions-approved-filters desktop-finance-table" aria-label="Φίλτρα συναλλαγών desktop">
        ...
        <label className="filter-label"><span>Λογαριασμός</span><AppSelectInput aria-label="Φίλτρο λογαριασμού" value={account} onChange={e=>{setAccount(e.target.value);setPage(1)}}><option value="all">Όλοι οι λογαριασμοί</option>{accounts.map(a=><option value={a.id} key={a.id}>{accountDisplayName(data,a.id)}</option>)}</AppSelectInput></label>
        ...
        <label className="filter-label"><span>Τύπος</span><AppSelectInput aria-label="Φίλτρο τύπου κίνησης" value={type} onChange={e=>{setType(e.target.value);setPage(1)}}><option value="all">Όλοι οι τύποι</option>{types.map(value=><option key={value} value={value}>{eventKindLabel(value)}</option>)}</AppSelectInput></label>
        <label className="transactions-approved-date"><input aria-label="Από ημερομηνία" type="date" value={dateStart} min={range.start} max={range.end} onChange={e=>{setDateStart(e.target.value);setPage(1)}}/><span>–</span><input aria-label="Έως ημερομηνία" type="date" value={dateEnd} min={range.start} max={range.end} onChange={e=>{setDateEnd(e.target.value);setPage(1)}}/></label>
        <Tooltip label={`Σειρά: ${sortDirection==='desc'?'νεότερες πρώτα':'παλαιότερες πρώτα'}`} side="left"><button type="button" className="transactions-approved-filter-button" aria-label="Αλλαγή σειράς συναλλαγών" onClick={()=>setSortDirection(value=>value==='desc'?'asc':'desc')}><SlidersHorizontal size={17}/></button></Tooltip>
      </div>
      ...
      {!rows.length?<div className="empty-state transaction-empty-state">Δεν υπάρχουν κινήσεις για τα επιλεγμένα φίλτρα και την περίοδο.</div>:<>
        <div className={`transactions-approved-grid desktop-finance-table ${detailOpen?'':'detail-closed'}`}>
          <article className="transactions-ledger-panel">
            <div className="transactions-approved-table-wrap"><table className="transactions-approved-table"><caption className="sr-only">Συναλλαγές για {month}</caption><thead><tr><th scope="col"><span className="transactions-approved-check" aria-hidden="true"/></th><th scope="col">Κατηγορία</th><th scope="col">Περιγραφή</th><th scope="col">Λογαριασμός</th><th scope="col" aria-sort={sortDirection==='asc'?'ascending':'descending'}>Ημερομηνία</th><th scope="col" className="amount">Ποσό</th><th scope="col" className="balance">Υπόλοιπο</th><th scope="col"><span className="sr-only">Ενέργειες</span></th></tr></thead><tbody>{pageRows.map(r=>{const note=noteParts(r.note);const balance=rowBalance(r);const isSelected=detailOpen&&selected?.id===r.id;return <tr key={r.id} className={`${isSelected?'is-selected ':''}transaction-row kind-${r.kind}`} data-transaction-kind={r.kind} data-transaction-source={r.source} data-legacy-override={r.overridden?'true':undefined} tabIndex={0} aria-selected={isSelected} onClick={()=>{setSelectedId(r.id);setDetailOpen(true)}} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();setSelectedId(r.id);setDetailOpen(true)}}}><td><input className="transactions-approved-check" type="radio" name="desktop-transaction-selection" aria-label={`Επιλογή ${note.title}`} checked={isSelected} onChange={()=>{setSelectedId(r.id);setDetailOpen(true)}} onClick={event=>event.stopPropagation()}/></td><td><span className="transactions-category-cell"><FinanceIcon settings={data.state.settings} kind={r.kind} category={r.category} subcategory={r.subcategory} note={r.note} size={16}/><span>{categoryLabel(r)}</span></span></td><td><span className="transactions-description-main" title={r.note}>{note.title}</span></td><td><span className="transactions-account-name">{accountText(r.accountIds)}</span></td><td>{shortDate(r.date)}</td><td className={`amount ${amountClass(r)}`}>{amountPrefix(r)}{money.format(r.amount)}</td><td className="balance">{balance===null?'—':money.format(balance)}</td><td><Tooltip label={`Λεπτομέρειες: ${note.title}`} side="left"><button type="button" className="row-context-button" aria-label={`Προβολή λεπτομερειών ${note.title}`} onClick={event=>{event.stopPropagation();setSelectedId(r.id);setDetailOpen(true)}}><MoreHorizontal size={17}/></button></Tooltip></td></tr>})}</tbody></table></div>
            <footer className="transactions-ledger-footer"><span>Εμφανίζονται {rows.length?`${pageStart+1}–${Math.min(pageStart+pageSize,rows.length)}`:'0'} από {rows.length} συναλλαγές</span><nav className="transactions-pagination" aria-label="Σελιδοποίηση συναλλαγών"><button type="button" aria-label="Προηγούμενη σελίδα" disabled={safePage<=1} onClick={()=>setPage(value=>Math.max(1,value-1))}><ChevronLeft size={15}/></button>{visiblePages.map(number=><button type="button" key={number} className={safePage===number?'active':''} aria-current={safePage===number?'page':undefined} onClick={()=>setPage(number)}>{number}</button>)}<button type="button" aria-label="Επόμενη σελίδα" disabled={safePage>=pageCount} onClick={()=>setPage(value=>Math.min(pageCount,value+1))}><ChevronRight size={15}/></button></nav><label className="transactions-page-size"><span>Εμφάνιση ανά σελίδα</span><select aria-label="Συναλλαγές ανά σελίδα" value={pageSize} onChange={e=>{setPageSize(Number(e.target.value));setPage(1)}}><option value={14}>14</option><option value={25}>25</option><option value={50}>50</option></select></label></footer>
          </article>
