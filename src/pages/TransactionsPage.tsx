...
export function TransactionsPage({
  ...
  const filterControls=(mobile=false)=><div className={`${mobile?'mobile-transaction-filters':'desktop-finance-table'} transaction-filter-controls`} aria-label="Φίλτρα συναλλαγών" style={mobile?undefined:{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}}><label className="filter-label" style={mobile?undefined:{flex:'1 1 180px',minWidth:0}}><span>Τύπος</span><AppSelectInput aria-label="Φίλτρο τύπου κίνησης" value={type} onChange={e=>setType(e.target.value)}><option value="all">Όλοι οι τύποι</option>{types.map(value=><option key={value} value={value}>{eventKindLabel(value)}</option>)}</AppSelectInput></label><label className="filter-label" style={mobile?undefined:{flex:'1 1 220px',minWidth:0}}><span>Λογαριασμός</span><AppSelectInput aria-label="Φίλτρο λογαριασμού" value={account} onChange={e=>setAccount(e.target.value)}><option value="all">Όλοι οι λογαριασμοί</option>{accounts.map(a=><option value={a.id} key={a.id}>{accountDisplayName(data,a.id)}</option>)}</AppSelectInput></label></div>;
