...
function ApprovedGroup({title,subtitle,tone,items,data,visible,asOf,onAction,onSnooze,onDismiss,emptyText}:{
  ...
  return <section className={`attention-approved-group ${tone}`} aria-labelledby={`attention-${tone}-title`}>
    <header className="attention-approved-group-head">
      ...
      {items.length?<button type="button" className="attention-approved-group-link" onClick={()=>document.getElementById(rowsId)?.scrollIntoView({block:'nearest'})}>Προβολή όλων <ArrowRight/></button>:<span className="attention-approved-group-count" aria-label="0 στοιχεία">0</span>}
    </header>
    ...
    {items.length?<div id={rowsId} className="attention-approved-rows" role="list">{items.map(item=><article role="listitem" className={`attention-row attention-approved-row ${item.severity} ${hasAmounts?'has-amount':'no-amount'}`} data-attention-id={item.id} key={item.id}>
...
export function AttentionPage({data,asOf,onAction,onDecision,onReviewDecision}:{data:FinanceData;asOf:string;onAction:(item:AttentionItem)=>void;onDecision:(id:string,decision:AttentionDecision)=>void;onReviewDecision:(id:string,decision:ReviewDecision)=>void}){
  ...
  return <div className="page-stack attention-page attention-approved-page">
    <div className="attention-approved-desktop">
      ...
      <ApprovedGroup title="Επείγοντα" subtitle="Άμεσα θέματα που απαιτούν την προσοχή σου." tone="danger" items={groups.danger} data={data} visible={visible} asOf={asOf} onAction={onAction} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχουν επείγοντα θέματα αυτή τη στιγμή."/>
      <ApprovedGroup title="Σύντομα" subtitle="Θέματα που χρειάζονται προγραμματισμό τις επόμενες ημέρες." tone="warning" items={groups.warning} data={data} visible={visible} asOf={asOf} onAction={onAction} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχει κάτι που λήγει σύντομα."/>
      <ApprovedGroup title="Εκκρεμότητες" subtitle="Γνωστές ενέργειες χαμηλότερης προτεραιότητας που παραμένουν ενεργές." tone="pending" items={groups.pending} data={data} visible={visible} asOf={asOf} onAction={onAction} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχουν άλλες ενεργές εκκρεμότητες."/>
      <ApprovedGroup title="Ενημερώσεις" subtitle="Πληροφοριακά σήματα από τα αποθηκευμένα δεδομένα και τις ντετερμινιστικές προβλέψεις." tone="notice" items={groups.notices} data={data} visible={visible} asOf={asOf} onAction={onAction} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχουν ξεχωριστές πληροφοριακές ενημερώσεις αυτή τη στιγμή."/>
