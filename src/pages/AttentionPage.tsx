...
const kindLabel=(kind:AttentionItem['kind'])=>kind==='scheduled'?'Προγραμματισμένη':kind==='recurring'?'Πάγιο':kind==='recurring_expiry'?'Λήξη παγίου':kind==='loan'?'Δόση δανείου':kind==='credit'?'Πιστωτική':kind==='lending'?'Δανεικά / Οφειλές':kind==='forecast'?'Πρόβλεψη':kind==='budget'?'Budget':kind==='account_balance'?'Υπόλοιπο':kind==='duplicate'?'Αντιστοίχιση':'Συναλλαγή';
const sourceLabel=(item:AttentionItem)=>item.kind==='scheduled'?'Προγραμματισμός':item.kind==='recurring'||item.kind==='recurring_expiry'?'Πάγια':item.kind==='loan'?'Δόσεις & Δάνεια':item.kind==='credit'?'Πιστωτική':item.kind==='lending'?'Δανεικά / Οφειλές':item.kind==='forecast'?'Πρόβλεψη 30 ημερών':item.kind==='budget'?'Προϋπολογισμοί':item.kind==='account_balance'?'Λογαριασμοί':'Συναλλαγές';
const itemContext=(data:FinanceData,item:AttentionItem)=>item.accountId?accountDisplayName(data,item.accountId):item.person||sourceLabel(item);
...
const KindIcon=({kind}:{kind:AttentionItem['kind']})=>kind==='scheduled'?<CalendarClock/>:kind==='recurring'||kind==='recurring_expiry'?<RefreshCw/>:kind==='loan'?<WalletCards/>:kind==='credit'?<ReceiptText/>:kind==='lending'||kind==='account_balance'?<Landmark/>:kind==='forecast'?<CircleAlert/>:kind==='duplicate'?<ListChecks/>:<FileText/>;
...
function ApprovedGroup({title,subtitle,tone,items,data,visible,asOf,onAction,onSnooze,onDismiss,emptyText}:{
  title:string;
  subtitle:string;
...
export function AttentionPage({data,asOf,onAction,onDecision,onReviewDecision}:{data:FinanceData;asOf:string;onAction:(item:AttentionItem)=>void;onDecision:(id:string,decision:AttentionDecision)=>void;onReviewDecision:(id:string,decision:ReviewDecision)=>void}){
  ...
  const activate=(item:AttentionItem)=>{
    ...
    if(item.action==='categorize_transaction'||item.action==='review_duplicate'){
      if(item.transactionId&&item.transactionSource){
        try{const url=new URL(location.href);url.searchParams.set('attentionTx',item.transactionId);url.searchParams.set('attentionSource',item.transactionSource);url.hash='#/transactions';history.pushState(history.state,'',url.toString());window.dispatchEvent(new PopStateEvent('popstate'));return}catch{}
      }
  ...
  return <div className="page-stack attention-page attention-approved-page">
    <div className="attention-approved-desktop">
      ...
      <ApprovedGroup title="Επείγοντα" subtitle="Άμεσα θέματα που απαιτούν την προσοχή σου." tone="danger" items={groups.danger} data={data} visible={visible} asOf={asOf} onAction={activate} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχουν επείγοντα θέματα αυτή τη στιγμή."/>
      <ApprovedGroup title="Σύντομα" subtitle="Θέματα που χρειάζονται προγραμματισμό τις επόμενες ημέρες." tone="warning" items={groups.warning} data={data} visible={visible} asOf={asOf} onAction={activate} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχει κάτι που λήγει σύντομα."/>
      <ApprovedGroup title="Εκκρεμότητες" subtitle="Γνωστές ενέργειες χαμηλότερης προτεραιότητας που παραμένουν ενεργές." tone="pending" items={groups.pending} data={data} visible={visible} asOf={asOf} onAction={activate} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχουν άλλες ενεργές εκκρεμότητες."/>
      <ApprovedGroup title="Ενημερώσεις" subtitle="Πληροφοριακά σήματα από τα αποθηκευμένα δεδομένα και τις ντετερμινιστικές προβλέψεις." tone="notice" items={groups.notices} data={data} visible={visible} asOf={asOf} onAction={activate} onSnooze={item=>decide(item,'snooze')} onDismiss={item=>decide(item,'dismiss')} emptyText="Δεν υπάρχουν ξεχωριστές πληροφοριακές ενημερώσεις αυτή τη στιγμή."/>
