...
import { CalendarClock, CircleCheck, CreditCard, Eye, EyeOff, HandCoins, Landmark, ListChecks, PiggyBank, TriangleAlert, WalletCards } from 'lucide-react';
...
export function ReportsPage({data,month,onUpsertBudget,onDeleteBudget,onUpsertRule,onDeleteRule}:{data:FinanceData;month:string;onUpsertBudget:(budget:MonthlyBudget)=>void;onDeleteBudget:(id:string)=>void;onUpsertRule:(rule:TransactionRule)=>void;onDeleteRule:(id:string)=>void}){
 ...
 return <div className="page-stack reports-dashboard reports-composite reports-dense">
  ...
  <section className="report-support-grid report-support-grid-four" id="report-support">
   ...
   <article className="panel neo-raised report-upcoming-card"><div className="panel-head"><div><span>Επερχόμενες υποχρεώσεις</span><small>Πάγια και επόμενες δόσεις.</small></div><CalendarClock aria-hidden="true"/></div>{upcomingObligations.length?<div className="report-upcoming-list">{upcomingObligations.map(row=><div key={row.id}>{row.kind==='loan'?<Landmark size={17}/>:<FinanceIcon settings={data.state.settings} kind="expense" category={row.category} note={row.name} size={17}/>}<span><b>{row.name}</b><small>{row.date?shortDate(row.date):'Επόμενη δόση'} · {row.category}</small></span><strong>{money.format(row.amount)}</strong></div>)}</div>:<div className="empty-state">Δεν υπάρχουν επερχόμενες υποχρεώσεις.</div>}<div className="report-credit-summary"><CreditCard size={16}/><span>Πιστωτικές</span><b>{money.format(snapshot.creditDebt)}</b><small>{creditPercent===null?'χωρίς συνολικό όριο':`${Math.round(creditPercent*100)}% χρήση`}</small></div></article>
  </section>
