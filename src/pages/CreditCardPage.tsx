...
export function CreditCardPage({
  ...
  })).filter(item=>item.events.length>0),[data,deletedCredit]);
  ...
  return <div className="page-stack credit-card-redesign-page">
    ...
    {deletedHistory.length?<section className="panel neo-raised deleted-credit-history" aria-labelledby="deleted-credit-history-title"><div className="panel-head"><div><span id="deleted-credit-history-title">Ιστορικό διαγραμμένων πιστωτικών</span><small>Read-only οικονομικό ιστορικό. Δεν διατηρούνται nickname, τράπεζα, last4 ή προστατευμένα στοιχεία της διαγραμμένης κάρτας.</small></div><ArchiveRestore/></div>{deletedHistory.map(({reference,events})=><article className="deleted-credit-history-card" key={reference.id}><div className="panel-head"><div><span>Διαγραμμένη κάρτα</span><small>Διαγράφηκε {shortDate(reference.deletedAt.slice(0,10))} · {events.length} {events.length===1?'ιστορική κίνηση':'ιστορικές κινήσεις'}</small></div></div><div className="semantic-table-wrap"><table className="semantic-table"><caption className="sr-only">Ιστορικές κινήσεις διαγραμμένης πιστωτικής κάρτας</caption><thead><tr><th>Ημερομηνία</th><th>Τύπος</th><th>Περιγραφή</th><th className="amount">Ποσό</th></tr></thead><tbody>{events.map(event=><tr key={event.id}><td>{shortDate(event.date)}</td><td>{event.kind==='card_purchase'?'Αγορά':'Αποπληρωμή'}</td><td>{event.note||'—'}</td><td className={`amount ${event.kind==='card_purchase'?'negative':''}`.trim()}>{money.format(event.amount)}</td></tr>)}</tbody></table></div></article>)}</section>:null}
    ...
    <ConfirmDialog open={Boolean(deleteCardTarget)} title="Ολική διαγραφή πιστωτικής;" description="Η κάρτα είναι αρχειοθετημένη και έχει μηδενική οφειλή. Θα διαγραφούν οριστικά το card profile, PAN, λήξη και CVV. Οι παλιές αγορές και αποπληρωμές δεν θα διαγραφούν και θα εμφανίζονται με ουδέτερη αναφορά «Διαγραμμένη κάρτα»." confirmLabel="Ολική διαγραφή" tone="destructive" busy={deleteCardBusy} motionMode={data.state.settings.motion} onConfirm={()=>void confirmCardDelete()} onCancel={()=>{if(!deleteCardBusy)setDeleteCardTarget(null)}}/>
  </div>;
}
