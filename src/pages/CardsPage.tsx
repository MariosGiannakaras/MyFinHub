import { ArchiveRestore, CreditCard, Plus, ShieldCheck, X } from 'lucide-react';
...
export function CardsPage({
  ...
  const restore=(card:PaymentCard)=>{onUpsertCard(restoreCard(card));setMessage(`Η «${card.nickname}» επανήλθε με το ίδιο ιστορικό και card id.`)};
  ...
  return <div className="page-stack cards-prototype-page">
    ...
    <CardCreateDialog open={Boolean(cardBank)} data={data} banks={cardBank?[cardBank]:banks.slice(0,1)} initialBankId={cardBank?.id} onClose={()=>setCardBankId(null)} onSave={card=>{onUpsertCard(card);setMessage(card.active?`Η «${card.nickname}» είναι διαθέσιμη σε Κάρτες και Πιστωτική όπου εφαρμόζεται.`:'')}}/>
