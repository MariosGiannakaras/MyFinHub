...
export function PlanningPage({ data, asOf, onUpsertScheduled, onCompleteScheduled }: {
  ...
  const setLifecycle = (item: ScheduledTransaction, status: 'skipped' | 'cancelled') => {
    const verb = status === 'cancelled' ? 'ακυρωθεί' : 'παραλειφθεί';
    if (!window.confirm(`Να ${verb} η προγραμματισμένη κίνηση «${item.note}»; Θα παραμείνει στο ιστορικό και δεν θα επηρεάσει υπόλοιπα.`)) return;
