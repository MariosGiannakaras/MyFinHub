...
export function PersistenceNotice({saveState,onRecover}:{saveState:SaveState;onRecover:()=>void}){
  ...
  if(saveState==='error'||saveState==='conflict'){
    ...
    return <div className={`persistence-notice ${saveState}`} role="alert" aria-live="assertive"><div><b>{conflict?'Υπάρχουν νεότερα δεδομένα':'Η αποθήκευση δεν ολοκληρώθηκε'}</b><small>{conflict?'Υπάρχει νεότερη αποθηκευμένη έκδοση. Η αποθήκευση σταμάτησε για να μη γραφτεί πάνω της κατά λάθος. Φόρτωσε την τελευταία έκδοση πριν συνεχίσεις.':'Η τελευταία αλλαγή δεν έχει επιβεβαιωθεί ως αποθηκευμένη. Φόρτωσε την τελευταία αποθηκευμένη έκδοση πριν συνεχίσεις.'}</small></div><Button variant="secondary" onClick={onRecover}>Φόρτωση τελευταίας έκδοσης</Button></div>;
  }
