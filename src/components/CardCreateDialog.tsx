...
export function CardCreateDialog({
  ...
  const submit=()=>{
    const name=nickname.trim();if(!name){setError('Συμπλήρωσε όνομα κάρτας.');return}if(!selected){setError('Επίλεξε σχέδιο / χρώμα κάρτας.');return}
  ...
  return <div className="picker-backdrop open prototype-card-picker" aria-hidden="false" onMouseDown={onClose}>
    <section ref={ref} className="picker card-create-modal neo-raised" role="dialog" aria-modal="true" aria-labelledby="card-create-title" tabIndex={-1} onMouseDown={event=>event.stopPropagation()} onKeyDown={event=>{if(event.key==='Escape'){event.preventDefault();onClose()}}}>
      ...
      <div className="modal-form-grid">
        {banks.length>1?<div className="modal-field"><label>Τράπεζα</label><select value={bank.id} onChange={event=>changeBank(event.target.value)}>{banks.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div>:null}
        ...
        {!kindLock?<div className="modal-field"><label>Τύπος</label><select value={kind} onChange={event=>setKind(event.target.value as CardKind)}><option value="debit">Χρεωστική</option><option value="credit">Πιστωτική</option><option value="prepaid">Προπληρωμένη</option></select></div>:null}
        <div className="modal-field"><label>Δίκτυο</label><select value={network} onChange={event=>setNetwork(event.target.value as CardNetwork)}><option value="visa">Visa</option><option value="mastercard">Mastercard</option></select></div>
