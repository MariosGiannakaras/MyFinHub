...
export function CardCreateDialog({
  ...
  const selected=designs.find(item=>item.id===designId)??designs[0];
  ...
  useEffect(()=>{
    ...
    const options=designsForBank(nextBank);const preferred=options.find(item=>item.kind===(kindLock||'debit'))??options[0];
  ...
  const preview:PaymentCard={id:'preview',bankId:bank.id,nickname:nickname.trim()||'Όνομα κάρτας',kind,network,formFactor:selected?.formFactor,designId:selected?.id,holderName:holder.trim()||undefined,last4:last4.replace(/\D/g,'')||undefined,active:true,createdAt:'',updatedAt:''};

  const pickDesign=(id:string)=>{
    const item=designs.find(option=>option.id===id);if(!item)return;
    setDesignId(item.id);if(!kindLock)setKind(item.kind);setNetwork(item.network);
  };
  const submit=()=>{
    ...
    <section ref={ref} className="panel neo-raised editor-dialog card-create-dialog" role="dialog" aria-modal="true" aria-labelledby="card-create-title" tabIndex={-1} onMouseDown={event=>event.stopPropagation()} onKeyDown={event=>{if(event.key==='Escape'){event.preventDefault();onClose()}}}>
      <div className="card-create-preview-stage" aria-label="Προεπισκόπηση νέας κάρτας">
        <article className={`r-payment-card is-preview ${cardThemeClass(preview)}`}>
          <div className="r-card-inner"><header className="r-card-head"><div className="r-card-brand"><BankBrandMark id={bank.id} name={bank.name} compact={false}/><div><b>{bank.name}</b><small>{preview.nickname}</small></div></div></header><div className="r-card-spacer"/><div className="r-card-number-line"><span>{digitsPreview(last4)}</span></div><div className="r-card-fields"><div><small>VALID THRU</small><span>MM/YY</span></div><div><small>CVV · LOCAL</small><span>•••</span></div><div className="r-card-network"><b>{network==='mastercard'?'Mastercard':network==='visa'?'Visa':'Card'}</b><small>{kindLock==='credit'||kind==='credit'?'Πιστωτική':kind==='prepaid'?'Prepaid':'Χρεωστική'}{selected?.formFactor==='virtual'?' · Virtual':''}</small></div></div></div>
        </article>
      </div>
      ...
      <div className="settings-form editor-grid card-create-fields">
        {banks.length>1?<label><span>Τράπεζα</span><select value={bank.id} onChange={event=>{const next=event.target.value;setBankId(next);const options=designsForBank(next);const preferred=options.find(item=>item.kind===(kindLock||kind))??options[0];setDesignId(preferred?.id||'');if(preferred)setNetwork(preferred.network)}}>{banks.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>:null}
      ...
      <div className="card-design-field"><span>Σχέδιο / χρώμα</span><div className="card-design-grid" role="radiogroup" aria-label="Σχέδιο κάρτας">{designs.filter(item=>!kindLock||item.kind===kindLock).map(item=><button key={item.id} type="button" role="radio" aria-checked={selected?.id===item.id} className="card-design-option" onClick={()=>pickDesign(item.id)}><i style={{background:item.swatch}}/><b>{item.label}</b><small>{item.note}</small></button>)}</div></div>
      ...
      <div className="editor-actions"><button type="button" className="secondary" onClick={onClose}>Ακύρωση</button><button type="button" className="save-button" onClick={submit}><Plus/> {archivedCardMatch(data,{bankId:bank.id,kind:kindLock||kind,last4:last4||undefined})?'Επαναφορά κάρτας':'Προσθήκη κάρτας'}</button></div>
    </section>
  </div>;
}
