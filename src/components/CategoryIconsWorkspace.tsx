...
type CategoryWorkspaceView='all'|'taxonomy'|'icons';
...
export function CategoryIconsWorkspace({data,asOf,settings,onChange,onTaxonomyOperation,view='all'}:{data:FinanceData;asOf:string;settings:FinanceSettings;onChange:(settings:FinanceSettings)=>void;onTaxonomyOperation:(operation:TaxonomyOperation)=>void;view?:CategoryWorkspaceView}){
  ...
  const showTaxonomy=view!=='icons';
  const showIcons=view!=='taxonomy';
  const title=view==='taxonomy'?'Κατηγορίες':view==='icons'?'Εικονίδια κατηγοριών':'Κατηγορίες & εικονίδια';
  ...
  return <section className="panel neo-raised category-icons-workspace" data-workspace-view={view} aria-labelledby="category-icons-title">
    ...
    {showTaxonomy?<div className="taxonomy-add-row" role="group" aria-label={`Νέα κατηγορία ${noun}`}>
      ...
      <Button type="button" variant="primary" onClick={addCategory}><Plus size={16} aria-hidden="true"/> Προσθήκη</Button>
    </div>:null}
    {showTaxonomy&&error?<div className="form-error taxonomy-error" role="alert">{error}</div>:null}
    {showTaxonomy&&retirement?.dependencies.length?<div className="logic-note compact taxonomy-retirement-blockers" role="status" aria-live="polite" data-taxonomy-retirement-blockers>
      <Archive size={18} aria-hidden="true"/>
      <div><b>Η «{retirement.label}» δεν μπορεί ακόμη να αποσυρθεί.</b><p>Τακτοποίησε εσύ τις παρακάτω ενεργές ή μελλοντικές αναφορές. Δεν θα γίνει αυτόματη μεταφορά, διαγραφή ή αλλαγή σε «Άλλο».</p><ul>{retirement.dependencies.map(item=><li key={`${item.kind}:${item.id}`}>{item.label}</li>)}</ul><Button type="button" variant="secondary" onClick={()=>setRetirement(null)}>Κλείσιμο</Button></div>
    </div>:null}
    ...
    <div className="category-icon-category-list taxonomy-category-list">
      {tree.map((category,categoryIndex)=>{
        ...
        const categoryMeta=view==='taxonomy'
          ?`${category.subcategories.length} ${category.subcategories.length===1?'υποκατηγορία':'υποκατηγορίες'}`
          :view==='icons'
            ?(parentKey?'Προσαρμοσμένο εικονίδιο':'Χωρίς ρητή επιλογή εικονιδίου')
            :`${category.subcategories.length} ${category.subcategories.length===1?'υποκατηγορία':'υποκατηγορίες'} · ${parentKey?'προσαρμοσμένο εικονίδιο':'χωρίς ρητή επιλογή'}`;
        return <article className="category-taxonomy-card" key={categoryId} data-category-id={categoryId}>
          <div className="category-taxonomy-head">
            ...
            <div className="category-taxonomy-title"><b>{category.name}</b><small>{categoryMeta}</small></div>
            {showTaxonomy?<div className="taxonomy-row-actions" aria-label={`Ενέργειες κατηγορίας ${category.name}`}>
          ...
          {showTaxonomy&&editing?.type==='category'&&editing.id===categoryId?<div className="taxonomy-inline-editor" role="group" aria-label={`Μετονομασία κατηγορίας ${category.name}`}><AppTextInput autoFocus value={editing.value} onChange={event=>setEditing({...editing,value:event.target.value})} onKeyDown={event=>{if(event.key==='Enter'){event.preventDefault();saveEdit()}if(event.key==='Escape')setEditing(null)}}/><Button type="button" variant="primary" onClick={saveEdit}>Αποθήκευση</Button><Button type="button" variant="secondary" aria-label="Ακύρωση μετονομασίας" title="Ακύρωση μετονομασίας" onClick={()=>setEditing(null)}><X size={15} aria-hidden="true"/></Button></div>:null}
          ...
          <div className="taxonomy-subcategory-section">
            ...
            {showTaxonomy?<div className="taxonomy-add-row taxonomy-add-subcategory" role="group" aria-label={`Νέα υποκατηγορία στην ${category.name}`}>
              ...
              <Button type="button" variant="secondary" onClick={()=>addSubcategory(categoryId)}><Plus size={15} aria-hidden="true"/> Προσθήκη</Button>
            </div>:null}
            ...
            {category.subcategories.length?<div className="taxonomy-subcategory-list" role="list" aria-label={`Υποκατηγορίες ${category.name}`}>
              {category.subcategories.map((subcategory,subcategoryIndex)=>{
                ...
                return <article className="taxonomy-subcategory-row" role="listitem" key={subcategoryId} data-subcategory-id={subcategoryId}>
                  <div className="taxonomy-subcategory-main"><CategoryIconGlyph iconKey={resolved} size={17}/><div><b>{subcategory}</b><small>{view==='taxonomy'?`Υποκατηγορία της «${category.name}»`:override?'Δικό της εικονίδιο':`Κληρονομεί από «${category.name}»`}</small></div></div>
                  {showTaxonomy?<div className="taxonomy-row-actions" aria-label={`Ενέργειες υποκατηγορίας ${subcategory}`}>
                  ...
                  {showTaxonomy&&editing?.type==='subcategory'&&editing.id===subcategoryId?<div className="taxonomy-inline-editor taxonomy-subcategory-editor" role="group" aria-label={`Μετονομασία υποκατηγορίας ${subcategory}`}><AppTextInput autoFocus value={editing.value} onChange={event=>setEditing({...editing,value:event.target.value})} onKeyDown={event=>{if(event.key==='Enter'){event.preventDefault();saveEdit()}if(event.key==='Escape')setEditing(null)}}/><Button type="button" variant="primary" onClick={saveEdit}>Αποθήκευση</Button><Button type="button" variant="secondary" aria-label="Ακύρωση μετονομασίας" title="Ακύρωση μετονομασίας" onClick={()=>setEditing(null)}><X size={14} aria-hidden="true"/></Button></div>:null}

                  {showTaxonomy&&moving?.id===subcategoryId?<div className="taxonomy-move-editor" role="group" aria-label={`Μεταφορά υποκατηγορίας ${subcategory}`}><label><span>Μεταφορά σε</span><AppSelectInput value={moving.targetCategoryId} onChange={event=>setMoving({...moving,targetCategoryId:event.target.value})}>{otherCategories.map(target=><option key={target.id} value={target.id}>{target.name}</option>)}</AppSelectInput></label><Button type="button" variant="primary" onClick={saveMove}>Μεταφορά</Button><Button type="button" variant="secondary" aria-label="Ακύρωση μεταφοράς" title="Ακύρωση μεταφοράς" onClick={()=>setMoving(null)}><X size={14} aria-hidden="true"/></Button></div>:null}
