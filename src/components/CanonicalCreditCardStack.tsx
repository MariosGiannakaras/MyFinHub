...
export function CanonicalCreditCardStack({cards,banks,selectedCardId,onActiveCardChange,onArchiveCard}:{cards:PaymentCard[];banks:CardBank[];selectedCardId?:string;onActiveCardChange?:(cardId:string)=>void;onArchiveCard:(card:PaymentCard)=>void|Promise<void>;}){
  ...
  useEffect(()=>{
    ...
    async function loadSecrets(card:RuntimeCard){
      ...
      const result:Secrets={...server,cvv:server.cvv??local??undefined};secretsRef.current.set(card.id,result);return result;
    }
    ...
    function updateSecretDom(card:RuntimeCard,cardEl:HTMLElement,revealed:boolean){
      ...
      const button=cardEl.querySelector<HTMLButtonElement>('.reveal-btn');if(button){const label=revealed?'Απόκρυψη στοιχείων':'Εμφάνιση στοιχείων';button.innerHTML=icon(revealed?'eyeoff':'eye');button.setAttribute('aria-pressed',revealed?'true':'false');button.setAttribute('aria-label',label);button.setAttribute('title',label)}
    }
