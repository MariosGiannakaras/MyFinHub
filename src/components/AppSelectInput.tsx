...
import { Children, isValidElement, useId, useMemo, useState, type ReactElement, type ReactNode } from 'react';
...
export function AppSelectInput({value,onChange,children,disabled=false,'aria-label':ariaLabel,className='',...inputProps}:{value:string;onChange:(event:ChangeLike)=>void;children:ReactNode;disabled?:boolean;'aria-label'?:string;className?:string;[key:string]:unknown}){
  const [open,setOpen]=useState(false);const id=useId();const dialogId=`owned-select-${id.replace(/:/g,'')}`;const ref=useModalFocus<HTMLElement>(open,'[aria-selected="true"]');
  ...
  const move=(event:React.KeyboardEvent<HTMLElement>)=>{if(event.key==='Escape'){event.preventDefault();close();return}if(!['ArrowDown','ArrowUp','Home','End'].includes(event.key))return;event.preventDefault();const nodes=[...event.currentTarget.querySelectorAll<HTMLButtonElement>('[role="option"]:not(:disabled)')];if(!nodes.length)return;const current=Math.max(0,nodes.indexOf(document.activeElement as HTMLButtonElement));const next=event.key==='Home'?0:event.key==='End'?nodes.length-1:event.key==='ArrowDown'?Math.min(nodes.length-1,current+1):Math.max(0,current-1);nodes[next]?.focus()};
  const portal=open&&typeof document!=='undefined'?createPortal(<div className="owned-popover-backdrop" onMouseDown={close}><section ref={ref} id={dialogId} className="owned-popover owned-select-popover neo-raised" role="dialog" aria-modal="true" aria-label={ariaLabel||'Επιλογή'} tabIndex={-1} onMouseDown={event=>event.stopPropagation()} onKeyDown={move}><header><b>{ariaLabel||'Επιλογή'}</b><button type="button" className="icon-button" aria-label="Κλείσιμο επιλογών" onClick={close}><X size={17}/></button></header><div className="owned-option-list" role="listbox" aria-label={ariaLabel||'Επιλογές'}>{options.map(option=><button type="button" role="option" aria-selected={option.value===value} disabled={option.disabled} key={option.value} onClick={()=>choose(option.value)}><span>{option.label}</span>{option.value===value?<Check size={16}/>:null}</button>)}</div></section></div>,document.body):null;
