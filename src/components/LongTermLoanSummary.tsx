...
export function LongTermLoanSummary({data,onPayLoan,onOpenLoans}:{data:FinanceData;onPayLoan:(loanId:string)=>void;onOpenLoans:()=>void}){
  ...
  return <section className="panel neo-flat long-term-recurring" aria-labelledby="linked-loan-obligations-title">
    <div className="panel-head"><div><span id="linked-loan-obligations-title">Δάνεια με μηνιαία πληρωμή</span><small>Εμφανίζονται εδώ για την επόμενη πληρωμή, αλλά παραμένουν τα ίδια δάνεια που διαχειρίζεσαι στις Δόσεις & Δάνεια.</small></div><Landmark/></div>
    ...
    <button type="button" className="text-button" data-open-loans="true" onClick={onOpenLoans}>Πλήρης προβολή στις Δόσεις & Δάνεια</button>
  </section>;
}
