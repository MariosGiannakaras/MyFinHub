    ...
    <div className="account-metadata-list">
      {accounts.map(account=>{
        ...
        return <div className="account-metadata-row" key={account.id} data-account-metadata-row={account.id}>
          ...
          <Button variant="primary" type="button" disabled={Boolean(busy[account.id])||invalid||!dirty[account.id]} onClick={()=>void save(account.id)}><Save size={15} aria-hidden="true"/> Αποθήκευση</Button>
