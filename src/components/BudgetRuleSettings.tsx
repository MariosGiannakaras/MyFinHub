...
export function BudgetRuleSettings({data,asOf,onUpsertBudget,onDeleteBudget,onUpsertRule,onDeleteRule}:{data:FinanceData;asOf:string;onUpsertBudget:(budget:MonthlyBudget)=>void;onDeleteBudget:(id:string)=>void;onUpsertRule:(rule:TransactionRule)=>void;onDeleteRule:(id:string)=>void}){
  ...
  return <div className="budget-rule-settings">
    ...
    <section className="panel neo-raised rule-settings-panel"><div className="panel-head"><div><span>Κανόνες συναλλαγών</span><small>First match wins: μικρότερη προτεραιότητα πρώτα, μετά σταθερό id. Οι κανόνες εφαρμόζονται μόνο σε νέες κινήσεις και δεν ξαναγράφουν ιστορικό.</small></div><ListFilter/></div>
      ...
      <div className="rule-preview" role="status" aria-live="polite"><div className="logic-note compact"><ListFilter/><span>Προεπισκόπηση μόνο: {previewMatches.length} υπάρχουσες κινήσεις στο επιλεγμένο scope ταιριάζουν. Δεν αλλάζει καμία από αυτές.</span></div>{previewMatches.length?<ul>{previewMatches.slice(0,3).map(event=><li key={event.id}><span>{event.note}</span><b>{money.format(event.amount)}</b></li>)}</ul>:null}</div>{ruleError?<div className="form-error" role="alert">{ruleError}</div>:null}<div className="editor-actions">{editingRuleId?<button type="button" className="secondary" onClick={resetRule}>Ακύρωση επεξεργασίας</button>:null}<button type="button" className="save-button" onClick={saveRule}>{editingRuleId?'Ενημέρωση κανόνα':'Προσθήκη κανόνα'}</button></div>
