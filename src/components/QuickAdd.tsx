...
export type QuickPrefill = { note:string; amount:number; category?:string; accountId?:string };
...
const kinds: Array<{kind:EventKind; label:string; desc:string; icon:ReactNode}> = [
  ...
  {kind:'saving_cash_offset',label:'Αποταμίευσα',desc:'Μισθοδοσίας → Ταμιευτηρίου',icon:<PiggyBank/>},
...
export function QuickAdd({ open, data, asOf, initial, initialKind='expense', prefill=null, motionMode='system', onClose, onCreate, currentBalance }: { open:boolean; data:FinanceData; asOf:string; initial?:FinanceEvent|null; initialKind?:EventKind; prefill?:QuickPrefill|null; motionMode?:'system'|'reduced'|'full'; onClose:()=>void; onCreate:(event:FinanceEvent)=>void; currentBalance:(accountId:string)=>number }) {
  ...
  const reset=()=>{setAmount('');setNote('');setError('');setActualBalance('');setDirty(false)};
  const chooseKind=(next:EventKind)=>{
    const defaults=entryDefaults(next,data.state.settings,fallbackAccount);
    setKind(next);setError('');setAccountId(defaults.accountId);setCategory(defaults.category);mark();
  ...
  return <AnimatePresence>{open?<motion.div className="modal-backdrop" initial={reduce?false:{opacity:0}} animate={{opacity:1}} exit={reduce?undefined:{opacity:0}} onMouseDown={requestClose}>
    <motion.section ref={modalRef} className="quick-modal neo-raised" role="dialog" aria-modal="true" aria-labelledby="quick-add-title" aria-describedby="quick-add-description" tabIndex={-1} initial={reduce?false:{opacity:0,scale:.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={reduce?undefined:{opacity:0,scale:.98,y:8}} transition={{duration:reduce?0:.18}} onMouseDown={e=>e.stopPropagation()} onKeyDown={e=>{if(e.key==='Escape'){e.preventDefault();requestClose()}}}>
      <header><div><small>{initial?'EDIT EVENT':'SMART ENTRY'}</small><h2 id="quick-add-title">{initial?'Επεξεργασία κίνησης':'Τι έγινε;'}</h2><p id="quick-add-description">Διάλεξε την πραγματική ενέργεια. Το RheomIQ δημιουργεί τα σωστά ledger legs από πίσω.</p></div><button type="button" className="icon-button" aria-label="Κλείσιμο Smart Entry" onClick={requestClose}><X/></button></header>
      <div className="kind-grid" role="group" aria-label="Είδος οικονομικής κίνησης">{kinds.map(k=><button type="button" key={k.kind} className={kind===k.kind?'active':''} aria-pressed={kind===k.kind} onClick={()=>chooseKind(k.kind)}><span>{k.icon}</span><b>{k.label}</b><small>{k.desc}</small></button>)}</div>
      <div className="entry-body">
        {kind==='expense'?<div className="frequent-strip"><span>Συχνά</span>{frequent.slice(0,6).map(f=><button type="button" key={f.label} onClick={()=>{setNote(f.label);setAmount(String(f.lastAmount));setCategory(f.category||category);if(f.accountId)setAccountId(f.accountId);mark()}}>{f.label}<small>{money.format(f.lastAmount)}</small></button>)}</div>:null}
        <div className="form-grid">
          ...
          {!transferLike && kind!=='card_purchase'?<label><span>Λογαριασμός</span><select value={accountId} onChange={e=>{setAccountId(e.target.value);mark()}}>{accounts.map(a=><option key={a.id} value={a.id}>{accountDisplayName(data,a.id)}</option>)}</select></label>:null}
          {transferLike?<><label><span>Από</span><select value={from} onChange={e=>{setFrom(e.target.value);mark()}}>{accounts.map(a=><option key={a.id} value={a.id}>{accountDisplayName(data,a.id)}</option>)}</select></label>{kind!=='card_payment'?<label><span>Προς</span><select value={to} onChange={e=>{setTo(e.target.value);mark()}}>{accounts.map(a=><option key={a.id} value={a.id}>{accountDisplayName(data,a.id)}</option>)}</select></label>:null}</>:null}
          {!['transfer','withdrawal','saving_cash_offset','card_payment','reconciliation'].includes(kind)?<label><span>Κατηγορία</span><select value={category} onChange={e=>{setCategory(e.target.value);mark()}}>{categoryOptions.map(c=><option key={c}>{c}</option>)}</select></label>:null}
          ...
          <label className="wide"><span>Σχόλιο <em>προαιρετικό</em></span><input value={note} onChange={e=>{setNote(e.target.value);mark()}} placeholder="Επίλεξε συχνή κίνηση ή γράψε μόνο αν χρειάζεται"/></label>
        </div>
        {kind==='saving_cash_offset'?<div className="logic-note"><PiggyBank/><div><b>Η σωστή λογική αποταμίευσης</b><span>Τα μετρητά δεν αλλάζουν. Μειώνεται μόνο η Μισθοδοσίας και αυξάνεται ισόποσα ο Ταμιευτηρίου. Τα φυσικά μετρητά είναι το αντίκρισμα/αιτία της ενέργειας.</span></div></div>:null}
        {kind==='split'?<div className="split-box"><div className="split-head"><b>Επιμέρους ποσά</b><span>Σύνολο: {money.format(parts.reduce((s,p)=>s+Number(p.amount||0),0))}</span></div>{parts.map((p,i)=><div className="split-line" key={p.id}><input aria-label={`Περιγραφή μέρους ${i+1}`} placeholder="Περιγραφή" value={p.label} onChange={e=>{setParts(ps=>ps.map((x,j)=>j===i?{...x,label:e.target.value}:x));mark()}}/><select aria-label={`Κατηγορία μέρους ${i+1}`} value={p.category} onChange={e=>{setParts(ps=>ps.map((x,j)=>j===i?{...x,category:e.target.value}:x));mark()}}>{data.state.settings.expenseCategories.map(c=><option key={c}>{c}</option>)}</select><input aria-label={`Ποσό μέρους ${i+1}`} inputMode="decimal" placeholder="0,00" value={p.amount||''} onChange={e=>{setParts(ps=>ps.map((x,j)=>j===i?{...x,amount:Number(e.target.value.replace(',','.'))}:x));mark()}}/><button type="button" aria-label={`Αφαίρεση μέρους ${i+1}`} onClick={()=>{setParts(ps=>ps.filter((_,j)=>j!==i));mark()}}><X size={15}/></button></div>)}<button type="button" className="text-button" onClick={()=>{setParts(ps=>[...ps,{id:`p${Date.now()}`,label:'',category:data.state.settings.expenseCategories[0]||'Άλλο',amount:0}]);mark()}}>+ Προσθήκη μέρους</button></div>:null}
        {error?<div className="form-error" role="alert" aria-live="assertive">{error}</div>:null}
      </div>
      <footer><button type="button" className="secondary" onClick={requestClose}>Ακύρωση</button><button type="button" className="save-button" onClick={submit}><Check size={17}/> {initial?'Αποθήκευση αλλαγών':'Καταχώριση'}</button></footer>
    </motion.section>
