...
export function QuickAdd({ open, data, asOf, initial, initialKind='expense', prefill=null, motionMode='system', onClose, onCreate, currentBalance }: { open:boolean; data:FinanceData; asOf:string; initial?:FinanceEvent|null; initialKind?:EventKind; prefill?:QuickPrefill|null; motionMode?:'system'|'reduced'|'full'; onClose:()=>void; onCreate:(event:FinanceEvent)=>void; currentBalance:(accountId:string)=>number }) {
  ...
  const submit=()=>{
    try{
      ...
      const event=createEvent({kind,date,amount:kind==='reconciliation'?Math.abs((actual as number)-reconciliationBase(accountId)):numeric,note:note||labelForKind(kind),category,accountId,fromAccountId:from,toAccountId:to,person:person.trim(),parts:kind==='split'?parts:undefined,actualBalance:actual,currentBalance:reconciliationBase(accountId)});
    ...
    <motion.section ref={modalRef} className="quick-modal neo-raised" role="dialog" aria-modal="true" aria-labelledby="quick-add-title" aria-describedby="quick-add-description" tabIndex={-1} initial={reduce?false:{opacity:0,scale:.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={reduce?undefined:{opacity:0,scale:.98,y:8}} transition={{duration:reduce?0:.18}} onMouseDown={e=>e.stopPropagation()} onKeyDown={e=>{if(e.key==='Escape'){e.preventDefault();requestClose()}}}>
      ...
      <div className="entry-body">
        ...
        <div className="form-grid">
          ...
          {!['transfer','withdrawal','saving_cash_offset','card_payment','reconciliation'].includes(kind)?<><label><span>Κατηγορία</span><select value={category} onChange={e=>{setCategory(e.target.value);setSubcategory('');mark()}}>{categoryOptions.map(c=><option key={c}>{c}</option>)}</select></label>{subcategoryOptions.length?<label><span>Υποκατηγορία</span><select value={subcategory} onChange={e=>{setSubcategory(e.target.value);mark()}}><option value="">Χωρίς υποκατηγορία</option>{subcategoryOptions.map(value=><option key={value}>{value}</option>)}</select></label>:null}</>:null}
        ...
        {kind==='split'?<div className="split-box"><div className="split-head"><b>Επιμέρους ποσά</b><span>Σύνολο: {money.format(parts.reduce((s,p)=>s+Number(p.amount||0),0))}</span></div>{parts.map((p,i)=>{const subs=subcategoriesFor(data.state.settings,'expense',p.category);return <div className="split-line split-line-taxonomy" key={p.id}><input aria-label={`Περιγραφή μέρους ${i+1}`} placeholder="Περιγραφή" value={p.label} onChange={e=>{setParts(ps=>ps.map((x,j)=>j===i?{...x,label:e.target.value}:x));mark()}}/><select aria-label={`Κατηγορία μέρους ${i+1}`} value={p.category} onChange={e=>{setParts(ps=>ps.map((x,j)=>j===i?{...x,category:e.target.value,subcategory:undefined}:x));mark()}}>{genericCategoryTree(data.state.settings,'expense').map(c=><option key={c.name}>{c.name}</option>)}</select>{subs.length?<select aria-label={`Υποκατηγορία μέρους ${i+1}`} value={p.subcategory||''} onChange={e=>{setParts(ps=>ps.map((x,j)=>j===i?{...x,subcategory:e.target.value||undefined}:x));mark()}}><option value="">—</option>{subs.map(value=><option key={value}>{value}</option>)}</select>:null}<input aria-label={`Ποσό μέρους ${i+1}`} inputMode="decimal" placeholder="0,00" value={p.amount||''} onChange={e=>{setParts(ps=>ps.map((x,j)=>j===i?{...x,amount:Number(e.target.value.replace(',','.'))}:x));mark()}}/><button type="button" aria-label={`Αφαίρεση μέρους ${i+1}`} onClick={()=>{setParts(ps=>ps.filter((_,j)=>j!==i));mark()}}><X size={15}/></button></div>})}<button type="button" className="text-button" onClick={()=>{setParts(ps=>[...ps,{id:`p${Date.now()}`,label:'',category:genericCategoryTree(data.state.settings,'expense')[0]?.name||'Άλλο',amount:0}]);mark()}}>+ Προσθήκη μέρους</button></div>:null}
