...
export function AccountIban({accountId,masked=false}:{accountId:string;masked?:boolean}){
  ...
  const displayIban=formattedIban?(masked?`${formattedIban.slice(0,4)} •••• •••• ${formattedIban.slice(-4)}`:formattedIban):'';
  ...
  return <div className="account-iban" data-account-iban={accountId} data-account-iban-masked={masked?'true':undefined}>
    <span className="account-iban-label">IBAN</span>
    {metadata.loading&&!metadata.loaded?<span className="account-iban-value">Φόρτωση…</span>:iban?<span className="account-iban-value" title={formattedIban}>{displayIban}</span>:<span className="account-iban-value muted">Δεν έχει οριστεί</span>}
