...
export function LoginScreen({ onLogin, error }:{ onLogin:(email:string,password:string)=>Promise<boolean>; error:string }) {
  ...
  return <main className="login-screen">
    <section className="login-card neo-raised" aria-labelledby="login-title">
      ...
      <div className="login-shield"><ShieldCheck size={24}/><span>Single-owner encrypted session</span></div>
      ...
      <form onSubmit={submit} className="login-form">
        ...
        <button className="primary-action login-submit" type="submit" disabled={busy}>{busy?'Έλεγχος…':'Σύνδεση'}</button>
      </form>
      <small className="login-footnote">Η συνεδρία αποθηκεύεται σε HttpOnly cookie και δεν είναι προσβάσιμη από JavaScript.</small>
    </section>
  </main>;
}
