...
export function DesktopUpdatePanel() {
  ...
  const action = !bridge
    ...
    : state.status === 'available'
      ? <Button variant="primary" disabled={busy} onClick={() => void run(bridge.downloadUpdate)}><Download size={16} /> Λήψη ενημέρωσης</Button>
      : state.status === 'ready'
        ? <Button variant="primary" disabled={busy} onClick={() => void run(bridge.installUpdate)}><RotateCcw size={16} /> Εγκατάσταση & επανεκκίνηση</Button>
        : <Button variant="secondary" disabled={busy || state.status === 'installing'} onClick={() => void run(bridge.checkForUpdates)}><RefreshCw size={16} className={state.status === 'checking' ? 'is-spinning' : ''} /> Έλεγχος τώρα</Button>;
